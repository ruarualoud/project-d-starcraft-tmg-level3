...
Current v2 consumes the exact latest official bundle and MatchBinding together with Cleanup v5 and current Determine Initiative history. LegalSpace exposes one exact action to the First Player; Apply must exact-match a fresh server enumeration. It preserves the frozen semantic kernel's specific illegal-reason codes for Referee/UI explanation while rejecting forged history, lineage, extra fields and source drift at the current boundary. Frozen v1 stays byte-exact for its historical catalogue/runtime/replay and old-rules display and is never a silent current-room compatibility path. The structural action schema remains `hybrid_legal_space_v19`.
...
Current v2 additionally requires the exact current Start-of-Round v2 resolution/log and Movement `PhaseFirstActorChoice` provenance, then follows a hash-contiguous Supply lineage through every supported current Reserve Deploy or Disengage mutation. Multiple deployments consume the preceding deployment's Supply state; an unwitnessed reset, forged intermediate hash or forged marker holder fails closed. Apply exact-matches a freshly re-enumerated v2 action before using an explicit adapter to the byte-frozen v1 geometry/transition semantic kernel. Frozen v1 remains queryable for historical replay and old-rules display but is never a silent current-room path. Current Reserve Deploy raises the structural action schema to `hybrid_legal_space_v20`.
...
Current v2 requires the exact current Start-of-Round v2 resolution/log, Movement `PhaseFirstActorChoice` provenance and a hash-contiguous current-round Supply lineage before exposing a domain. Unit size is contract material: the live model denominator selects the official split Speed (`4` for multiple Marines, `7` for one) and fixes the exact remaining-placement count. Apply exact-matches a freshly re-enumerated v2 action, then invokes the byte-frozen v1 geometry/transition semantic kernel only through an explicit adapter with `silentCompatibilityUsed=false`. Frozen v1 remains available only through its exact historical catalogue/runtime and rules display. Current Standard Move raises the structural action schema to `hybrid_legal_space_v21`.
