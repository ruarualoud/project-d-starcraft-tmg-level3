...
Current v4 consumes the exact latest official bundle and MatchBinding together with Cleanup v5 and current Determine Initiative history. LegalSpace exposes one exact action to the First Player; Apply must exact-match a fresh server enumeration. In addition to the frozen v3 resolution semantics, v4 validates each supported Unit's selected upgrade loadout, permits only no upgrade or the exact official Marine `Stimpack` row in the bounded subset, and binds the ordered loadout rows and hash into the action, resolution, event, history and state before Movement opens. It also preserves the current round's empty, runtime-bound `SupplyLossLedger`. Frozen v1/v2/v3 remain byte-exact for their historical catalogue/runtime/replay and old-rules display and are never silent current-room compatibility paths. Current Movement authority uses structural action schema `hybrid_legal_space_v23`.
...
Current v4 requires the exact Start-of-Round v4 action/resolution/event/history and selected-loadout hash, its empty `SupplyLossLedger`, Movement `PhaseFirstActorChoice` provenance, and one hash-contiguous Supply lineage whose mutations use only current v4 executor identities. Multiple deployments consume the preceding mutation state; an unwitnessed reset, mixed historical/current identity, forged loadout/ledger/action/event hash or forged marker holder fails closed. Apply exact-matches a freshly re-enumerated v4 action, presents an explicitly validated no-upgrade view to the byte-frozen v1 geometry/transition semantic kernel, then restores the exact hash-bound loadout. Frozen v1/v2/v3 remain queryable only through their historical runtime and rules display. Current Movement authority uses `hybrid_legal_space_v23`.
...
Current v4 consumes the same shared Start v4 selected-loadout binding, Phase Initiative and current v4 Supply/SupplyLoss lineage as Reserve Deploy and Disengage. Unit size remains contract material: the live model denominator selects the official split Speed (`4` for multiple Marines, `7` for one) and fixes the exact remaining-placement count. Apply exact-matches a freshly re-enumerated v4 action, presents an explicitly validated no-upgrade view to the byte-frozen v1 geometry/transition semantic kernel, then restores the exact hash-bound loadout with `silentCompatibilityUsed=false`. Frozen v1/v2/v3 remain available only through exact historical catalogue/runtime/rules-display bindings. Current Movement authority uses `hybrid_legal_space_v23`.
