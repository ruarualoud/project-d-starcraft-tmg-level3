...
Status: active; 10/16 implementation slices complete, 6 remaining. The route
...
| 140 | Implement the complete LegalSpace-classified Token/Marker action palette and authoritative write-sheet entry points. | Generated rule-graph denominator across all create/place/move/consume/remove actions; source, legal geometry, controller, duration, stack/unique, trigger and cleanup shown; damage/status/casualty/deploy sheet edits route to LegalSpace; no unit-specific hard-coded whitelist or direct client mutation. |
