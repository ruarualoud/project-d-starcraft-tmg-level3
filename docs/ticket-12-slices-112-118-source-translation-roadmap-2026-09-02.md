...
Status: Slice 112 complete; Ticket 12 open at 1/7 planned slices.
...
| 113 | Bind rulebook/P2P/FAQ page locators, source precedence, source-scope review, and rights/redistribution decisions to every public provenance class. | Page/record coverage, precedence conflicts, unresolved-rights isolation, old-source display retention. |
| 114 | Define the explicit-command-only importer, immutable raw capture, version/diff classification, review queue, rollback, and room pinning workflow. | Offline replay of stored captures, same-version display-only drift, value/schema drift quarantine, no silent replacement. |
| 115 | Implement the real direct translation Provider Adapter and bounded prompt/glossary/cost/attempt receipts. DSH remains forbidden outside Skill generation. | Credential isolation, egress/model/profile binding, real Provider smoke, failure and retry accounting, canonical non-mutation. |
