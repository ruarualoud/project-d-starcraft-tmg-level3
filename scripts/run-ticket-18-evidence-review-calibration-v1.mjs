...
import { createStrategyEvidenceReviewContractV1, reconcileStrategyReviewOpinionsV1 } from '../packages/strategy-skills/strategy-evidence-review-v1.mjs';
...
for (const receipt of [fixture.originalRecipe, gate, previous]) for (const row of receipt.codeHashes) {
...
const contract = createStrategyEvidenceReviewContractV1(), { binding } = providerRegistry();
const limits = fixture.originalRecipe.limits;
const amendment = seal({ schema: 'ticket18_evidence_review_calibration_amendment_v1', parentRun: PARENT_RUN,
  ...
  maxAdditionalCalls: 8, maxAdditionalCostMicros: 5000000, inheritedLimits: limits,
  codeHashes: await codeHashes(['scripts/run-ticket-18-evidence-review-calibration-v1.mjs']),
  scope: 'one_new_protocol_probe_then_14_discrimination_decisions_then_11_field_audits_only_if_calibrated',
...
const out = path.join(BUILD, PARENT_RUN, 'evidence-review-calibration-v1');
...
const lease = store.acquire('evidence-review-amendment-v1', { amendmentHash: amendment.hash });
const checkpoint = lease.cached ? lease.artifact : store.finish(lease, seal({ amendment, initial }));
...
try {
  ...
  const reviewer = createStrategyEvidenceReviewerV1({ store: journal, dsh, providerAdapter: adapter,
    egressBinding: binding, capabilityReceipt: capability,
    executionPolicy: fixture.originalRecipe.executionPolicy, priceUsage });
  for (const group of ['current', 'mutant']) {
    for (const [index, prepared] of fixture[group === 'current' ? 'currentBatches' : 'mutantBatches'].entries()) {
      await save(group + '-input-' + index, prepared);
      const result = await reviewer.review(prepared); results[group].push(result);
      await save(group + '-result-' + index, result);
      console.log(JSON.stringify({ event: 'review-batch', group, index,
        verdicts: result.evidence.checks.map(c => ({ id: c.target.targetId, verdict: c.verdict })) }));
    }
  }
