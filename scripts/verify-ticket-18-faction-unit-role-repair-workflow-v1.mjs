...
try { actualRoles = new Map(db.prepare("SELECT id,artifact FROM steps WHERE run=? AND state='complete'")
  .all('faction-v1-c27e73d05132d0b01a17').map(r => [r.id, verifySeal(JSON.parse(r.artifact)).value])); }
finally { db.close(); }
const temp = await mkdtemp(path.join(base, 'unit-field-workflow-'));
...
async function run(negative) {
  ...
  const runtime = { async role(request) {
    ...
    if (request.roleId.startsWith(sectionId + '.known-unit-fields.')) {
      ...
      return seal({ roleId: id, fixtureOnly: true, output: { replacements: request.workspace.editTargetsAtEnd.map((t, n) =>
        ({ targetId: t.targetId, text: replacements[n] })) } });
    }
    if (request.roleId.startsWith(sectionId + '.review-target-batch-v1.') && request.roleId.includes('.1.')) {
      ...
      if (/^(supportive|adversarial)\.1\./.test(suffix)) {
        freshReviews++; allFreshCalls++; repairedHash = hash(request.workspace.draft);
        assert.equal(inspectFactionUnitRoleDebtV1({ input, draft: request.workspace.draft }).findings.length, 0);
    ...
    if (request.roleId.startsWith(sectionId + '.editor.1.')) fail('INJECTED_UNIT_FRESH_NEGATIVE_RETAINED');
    const saved = actualRoles.get(id); assert(saved?.roleId === id, id); return saved;
  } };
  try {
    await assert.rejects(produceFactionStrategyV1({ input, knownRulePolicy, fieldRepairSeed: { sourceSection, candidate, evidence }, runtime, store,
    ...
    assert.equal(repairs, 1); assert.equal(freshReviews, 8);
    const result = store.artifact(sectionId + '.result');
    if (negative) { assert.equal(completed, null); assert.equal(result, null); }
    else {
      assert(completed.passed); assert.equal(result.rounds.length, 2); assert.equal(hash(result.draft), repairedHash);
      assert.equal(result.edits.length, 1); assert.equal(result.edits[0].patch.changes.length, 3);
      assert.equal(result.edits[0].sourceReviewPassed, false); assert.equal(result.rounds.at(-1).draftHash, repairedHash);
...
const files = ['packages/skill-production-v3/faction-strategy-workflow-v1.mjs', 'packages/skill-production-v3/faction-unit-role-field-repair-v1.mjs',
  'packages/skill-evaluation/faction-unit-role-debt-v1.mjs', 'scripts/verify-ticket-18-faction-unit-role-repair-workflow-v1.mjs'];
const codeHashes = await Promise.all(files.map(async file => ({ file, hash: sha256(await readFile(path.join(root, file))) })));
const report = seal({ passed: true, checks: 8, codeHashes, inputHash: input.hash, knownCounterexamples: 3,
...
await writeFile(path.join(base, 'unit-role-repair-workflow-readiness.json'), JSON.stringify(report, null, 2));
