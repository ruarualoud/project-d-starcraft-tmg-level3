...
} from "../packages/rule-atoms/official-ranged-attack-executor-v5.mjs";
import {
  createOfficialRangedAttackRuleSliceV5,
  verifyOfficialRangedAttackRuleSliceV5,
...
const slice = createOfficialRangedAttackRuleSliceV5({
  previousSlice: previousReport.slice,
});
const sliceAudit = verifyOfficialRangedAttackRuleSliceV5({
  previousSlice: previousReport.slice,
  slice,
});
  ...
  return {
    ...
    pieces: [
      piece({
        ...
        currentSupply: 1,
        selectedUpgradeNames: input.raynorUpgradeNames ?? ["C-14 rifle"],
...
check("slice_promotes_five_atoms_and_preserves_v4_history", () => {
  assert.equal(OFFICIAL_RANGED_ATTACK_V5_NEW_ATOM_IDS.length, 5);
  assert.equal(sliceAudit.counts.executableRuleAtoms, 304);
  assert.equal(sliceAudit.counts.newlyExecutableRuleAtoms, 5);
  assert.equal(sliceAudit.counts.versionReassignedRuleAtoms, 60);
  assert.equal(sliceAudit.counts.reviewRequiredRuleAtoms, 608);
...
check("selected_c14_is_legal_with_burst_inside_eight_inches", () => {
  const candidates = enumerateOfficialRangedAttackActionsV5(state(), {
    sideKey: "player1",
    matchBinding,
  });
  assert.equal(candidates.length, 1);
  assert.equal(candidates[0].weaponName, "C-14 rifle");
  assert.equal(candidates[0].executorId, OFFICIAL_RANGED_ATTACK_V5_EXECUTOR_ID);
  assert.equal(candidates[0].executorVersion, OFFICIAL_RANGED_ATTACK_V5_EXECUTOR_VERSION);
  assert.deepEqual(candidates[0].chance.layout, { hit: 9, surge: 1, armour: 9, evade: 0 });
  assert.equal(candidates[0].chance.count, 19);
  assert.equal(candidates[0].details.burstFireApplied, true);
  assert.equal(candidates[0].details.printedRateOfAttack, 6);
  assert.equal(candidates[0].details.effectiveRateOfAttack, 9);
  assert.match(candidates[0].details.weaponLoadoutHash, /^[a-f0-9]{64}$/u);
});

check("outside_burst_distance_keeps_c14_legal_at_printed_roa", () => {
  const candidates = enumerateOfficialRangedAttackActionsV5(state({ targetXInches: 16 }), {
    sideKey: "player1",
    matchBinding,
  });
  assert.equal(candidates.length, 1);
  assert.deepEqual(candidates[0].chance.layout, { hit: 6, surge: 1, armour: 6, evade: 0 });
  assert.equal(candidates[0].chance.count, 13);
  assert.equal(candidates[0].details.burstFireApplied, false);
  assert.equal(candidates[0].details.effectiveRateOfAttack, 6);
  assert.deepEqual(enumerateOfficialRangedAttackActionsV5(state({ targetXInches: 20 }), {
    sideKey: "player1",
    matchBinding,
  }), []);
});

check("unselected_or_engaged_replacement_weapon_fails_closed", () => {
  assert.deepEqual(enumerateOfficialRangedAttackActionsV5(state({ raynorUpgradeNames: [] }), {
    sideKey: "player1",
    matchBinding,
  }), []);
  const missingUpgrade = enumerateOfficialRangedAttackActionsV5(
    state({ raynorUpgradeNames: [] }),
    { sideKey: "player1", matchBinding, includeDisabled: true },
  );
  assert.equal(missingUpgrade[0].disabledReason, "RANGED_ATTACK_V5_WEAPON_LOADOUT_SCOPE_UNSUPPORTED");
  assert.deepEqual(enumerateOfficialRangedAttackActionsV5(state({ targetXInches: 7 }), {
    sideKey: "player1",
    matchBinding,
  }), []);
});

check("exact_reveals_use_burst_roa_then_existing_surge_armour_damage", () => {
  const before = state();
  const candidate = enumerateOfficialRangedAttackActionsV5(before, {
    sideKey: "player1",
    matchBinding,
  })[0];
  const transition = applyOfficialRangedAttackV5(before, action(candidate), {
    matchBinding,
    postRevision: 1,
    chanceReveals: [
      3, 1, 1, 1, 1, 1, 1, 1, 1,
  ...
  assert.equal(event.burstFire.applied, true);
  assert.equal(event.burstFire.effectiveRateOfAttack, 9);
  assert.equal(event.weaponLoadout.availableWeaponNames.includes("Commando Rifle"), false);
  assert.equal(event.attackPool.dice, 9);
  assert.equal(event.attackPool.hits, 1);
  assert.equal(event.surgePool.bypassedArmourHits, 1);
...
check("authority_preview_apply_and_ed25519_replay_survive_hmac_rotation", () => {
  ...
  const engine = engineWithKeys(refereeKeys, "ticket-11-burst-fire-short-seal-v1");
  const initial = engine.createEnvelope({
    roomId: "official-burst-fire-authority-room",
    dataVersion:
    ...
    dependencies: {
      ...
      dataSnapshot: { artifactId: "official-burst-fire-gameplay-data-bundle", content: gameplayDataBundle },
    },
    state: state(),
  });
  ...
  assert.ok(ranged);
  assert.equal(ranged.action.executorId, OFFICIAL_RANGED_ATTACK_V5_EXECUTOR_ID);
  ...
  assert.equal(preview.preview.core.chanceTicket.tickets.length, 19);
  ...
  const applied = engine.apply({
    ...
    idempotencyKey: "ticket-11-burst-fire-apply-v1",
  });
  assert.equal(applied.ok, true);
  assert.equal(applied.receipt.action.executorId, OFFICIAL_RANGED_ATTACK_V5_EXECUTOR_ID);
  assert.equal(applied.receipt.chanceReveal.reveals.length, 19);
  assert.equal(applied.receipt.eligibleForTraining, false);

  const replayEngine = engineWithKeys(
    refereeKeys,
    "ticket-11-burst-fire-rotated-short-seal-v2",
  );
...
check("runtime_slice_latest_data_and_training_boundaries_are_exact", () => {
  ...
  assert.equal(runtime.descriptor.executableRuleAtomCount, 304);
  assert.equal(runtime.descriptor.productionRoomEligible, false);
  assert.equal(slice.effectKernel.knownUnimplementedEffectAtoms, 8);
...
const report = {
  schema: "starcraft_tmg_official_ranged_attack_rule_slice_v5_report",
