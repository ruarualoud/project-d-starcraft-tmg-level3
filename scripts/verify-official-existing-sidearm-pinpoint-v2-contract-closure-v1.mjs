...
import {
  createOfficialExistingSidearmPinpointV2ContractClosureRuleSliceV1,
  OFFICIAL_SLICE_72_MIGRATED_ATOM_IDS,
  verifyOfficialExistingSidearmPinpointV2ContractClosureRuleSliceV1,
} from "../packages/rule-atoms/official-existing-sidearm-pinpoint-v2-contract-closure-rule-slice-v1.mjs";
...
import { OFFICIAL_SIDEARM_PINPOINT_RANGED_BATCH_V2_EXECUTOR_ID } from
  "../packages/rule-atoms/official-sidearm-pinpoint-ranged-batch-executor-v2.mjs";
import { createOfficialSidearmPinpointV2RelationshipExtensionV1 } from
  "../packages/rule-atoms/official-sidearm-pinpoint-v2-relationship-contract-v1.mjs";
import * as fixture from "./verify-official-sidearm-pinpoint-v2-public-contract-v1.mjs";
...
const slice = createOfficialExistingSidearmPinpointV2ContractClosureRuleSliceV1({
  previousSlice: previousReport.slice,
});
const audit = verifyOfficialExistingSidearmPinpointV2ContractClosureRuleSliceV1({
  previousSlice: previousReport.slice,
  slice,
});
...
acceptance.push("six_existing_sidearm_atoms_rebound_without_denominator_growth");
...
const selected = candidates.find((candidate) => (
  candidate.attackProfileKey === "army_units:goliath::assault::Underbelly Machine Gun"
...
acceptance.push("current_runtime_exposes_20_candidates_and_settles_single_sidearm_batch");
...
function authorityEngine(hmacSecret) {
  return createStarcraftTmgAuthoritativeEngine({
    ...
    cryptoOptions: {
      keyId: "ticket-11-slice-72-referee-v1",
      privateKey,
      publicKey,
      hmacSecret,
    },
...
function credentials(engine, envelope) {
  const authority = engine.issueSeatAuthority({
    grantId: "slice-72-player1-grant",
  ...
  const lease = engine.issueControlLease({
    ...
    sessionId: "slice-72-player1-session",
    leaseFence: 1,
...
const engine = authorityEngine("ticket-11-slice-72-short-seal-v1");
const initialEnvelope = engine.createEnvelope({
  roomId: "official-slice-72-sidearm-pinpoint-room",
  ...
  dependencies: {
    ...
    dataSnapshot: {
      artifactId: "official-sidearm-pinpoint-gameplay-data-bundle",
...
const finite = legalSpace.finiteActions.find((entry) => (
  entry.action.executorId === OFFICIAL_SIDEARM_PINPOINT_RANGED_BATCH_V2_EXECUTOR_ID
    && entry.action.attackProfileKey
...
const authorityApplied = engine.apply({
  ...
  idempotencyKey: "slice-72-sidearm-pinpoint",
});
...
const replayEngine = authorityEngine("ticket-11-slice-72-rotated-short-seal-v2");
...
const graph = createRuleRelationshipGraphV1({
  ...
  extension: createOfficialSidearmPinpointV2RelationshipExtensionV1({
...
assert.equal(graphAudit.counts.declaredStateContractExecutors, 38);
assert.equal(graphAudit.counts.stateContractMissingExecutors, 4);
const brokenGraph = structuredClone(graph);
const targetScope = brokenGraph.coverageScopes.find((entry) => (
  entry.executorId === OFFICIAL_SIDEARM_PINPOINT_RANGED_BATCH_V2_EXECUTOR_ID
));
...
const oldExecutorBytes = await readFile(path.join(
  ROOT,
  "packages/rule-atoms/official-sidearm-pinpoint-ranged-batch-executor-v1.mjs",
));
assert.equal(sha256(oldExecutorBytes),
  "a085657bb805fad5c373dbc1bef1e24b157aaf82ac3d5633d7474eebdc4e3047");
...
acceptance.push("slice71_runtime_rules_display_and_v1_executor_bytes_remain_frozen");
...
const report = {
  schema:
    "starcraft_tmg_official_existing_sidearm_pinpoint_v2_contract_closure_verification_v1",
  ...
  rulesTruth: "official_current_sidearm_pinpoint_v2_contract_closure",
  trainingTruth: false,
};
