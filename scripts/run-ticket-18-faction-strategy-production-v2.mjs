...
import { createFactionWritingPlanV1, produceFactionStrategyV1, renderFactionStrategyV1 } from '../packages/skill-production-v3/faction-strategy-workflow-v2.mjs';
...
import { inspectFactionContinuationV2 } from '../packages/skill-production-v3/faction-continuation-v2.mjs';
...
import { classifyFactionFieldRecoveryOwnerActivityV1,
  loadFactionFieldRecoveryScopeV1 } from '../packages/skill-production-v3/faction-field-recovery-scope-v1.mjs';
...
import { createFactionBudgetExtensionV1, createFactionBudgetExtensionV2, createFactionBudgetExtensionV3,
  projectFactionCumulativeCostV2, verifyFactionCostNotificationV1 } from '../packages/skill-production-v3/faction-budget-extension-v1.mjs';
...
import { FACTION_OUTLINE_CAPACITY_BINDING_V1 as outlineCapacityBinding } from '../packages/skill-production-v3/faction-outline-capacity-envelope-v1.mjs';
import { withFactionOutlineCapacityRecoveryV1, usesFactionNativeProductionInputV2 } from '../packages/skill-production-v3/faction-outline-capacity-runtime-v1.mjs';
import { FACTION_NATIVE_ITEMS_ENVELOPE_BINDING_V1 as itemsEnvelopeBinding } from '../packages/skill-production-v3/faction-native-items-envelope-recovery-v1.mjs';
import { withFactionNativeItemsEnvelopeRecoveryV1,
  usesFactionNativeProductionInputV3 } from '../packages/skill-production-v3/faction-native-items-envelope-runtime-v1.mjs';
import { planFactionVolatileStructuralReissueV1,
  withFactionVolatileStructuralStoreV1,
  withFactionVolatileStructuralRuntimeV1 } from
...
import { FACTION_PARSED_REVIEW_VALUE_BINDING_V2 as parsedReviewNarrativeBinding }
  from '../packages/skill-production-v3/faction-parsed-review-value-v1.mjs';
import { FACTION_REVIEW_SOFT_LIMIT_BINDING_V1 as reviewSoftLimitBinding }
  from '../packages/skill-production-v3/faction-review-soft-limit-v1.mjs';
...
import { FACTION_EXECUTION_MODEL_BINDING_V1, FACTION_EXECUTION_MODEL_FILES_V1, factionExecutionProfileV1,
  factionExecutionEgressV1, factionProfileRefV1, assertFactionExecutionModelNewSendV1,
  estimateFactionBetaUsageCnyMicrosV1 } from '../packages/skill-production-v3/faction-execution-model-v1.mjs';
import { FACTION_MODEL_LIFECYCLE_BINDING_V1 as modelLifecycleBinding,
  selectFactionExecutionModelV1 } from '../packages/skill-production-v3/faction-model-lifecycle-v1.mjs';
import { createFactionProductionReleaseManifestV1,
  restoreFactionProductionContinuationV1 } from
...
const requestStableRelease = args.includes('--stable-release-v1');
if (requestStableRelease) args.splice(args.indexOf('--stable-release-v1'), 1);
...
if (requestStableRelease && (args.length !== 5 || requestBudgetEpoch
  || requestBetaModel || requestBudgetExtensionV3 || requestBudgetExtensionV2
  || requestReviewTransaction)) fail('FACTION_STABLE_RELEASE_ARGUMENTS_INVALID');
...
const assertLegacyReadinessCodeHashes = async (rows, code) => {
  if (requestStableRelease) return;
  for (const row of rows) if (sha256(await readFile(path.join(root, row.file))) !== row.hash) fail(code);
};
const executionModelReadiness = await json('build/ticket-18-faction-production-v1/execution-model-readiness-v1.json');
if (requestBetaModel) {
  ...
  await assertLegacyReadinessCodeHashes(executionModelReadiness.codeHashes,
    'FACTION_EXECUTION_MODEL_READINESS_CODE_DRIFT');
}
...
// A fully produced release is a read path, not a production session. Resolve
// it before dependency reconstruction, DSH preparation, Keychain access or
// capability renewal. Partial releases deliberately fall through to the
// section-snapshot production path below.
if (requestStableRelease) {
  const releasedRecipe = await json('build/ticket-18-faction-production-v1/'
    + args[4] + '/recipe.json');
  const releasedCandidates = [];
  for (const name of ['terran_armed_forces', 'zerg_swarm']) {
    try {
      releasedCandidates.push(await json(
        'build/ticket-18-faction-production-v1/' + args[4] + '/'
        + name + '-candidate.json'));
    } catch (error) {
      if (error.code !== 'ENOENT') throw error;
    }
  }
  if (releasedCandidates.length === 2) {
    const generalSkill = await json(
      'build/ticket-18-general-strategy-live-v1/'
      + 'general-strategy-3705e4aa46ecd74a7826207a67b5b096/'
      + 'general-strategy-final-v1/final-general-skill.json');
    const releaseDb = new DatabaseSync(filename, { readOnly: true });
    let checkpointSummary;
    try {
      const registered = releaseDb.prepare(
        'SELECT recipe FROM runs WHERE id=?').get(args[4]);
      if (!registered || registered.recipe !== releasedRecipe.hash)
        fail('FACTION_PRODUCTION_RELEASE_REGISTERED_RUN_DRIFT');
      if (releaseDb.prepare(
        "SELECT count(*) AS n FROM attempts WHERE state='intent'").get().n)
        fail('AMBIGUOUS_EGRESS_NO_RETRY');
      if (releaseDb.prepare(
        "SELECT count(*) AS n FROM attempts WHERE code='PROVIDER_PAYMENT_REQUIRED'").get().n)
        fail('API_BALANCE_EXHAUSTED_STOP_ALL_WORK');
      if (releaseDb.prepare(
        "SELECT count(*) AS n FROM steps WHERE run=? AND state='running'")
        .get(args[4]).n)
        fail('FACTION_PRODUCTION_RELEASE_CHECKPOINT_RUNNING');
      const rows = releaseDb.prepare(
        'SELECT state,count(*) AS n FROM steps WHERE run=? GROUP BY state')
        .all(args[4]);
      checkpointSummary = {
        complete: rows.find(row => row.state === 'complete')?.n || 0,
        pending: rows.find(row => row.state === 'pending')?.n || 0,
      };
    } finally {
      releaseDb.close();
    }
    const release = createFactionProductionReleaseManifestV1({
      runId: args[4], recipe: releasedRecipe,
      currentRecipe: releasedRecipe,
      existingSkills: [generalSkill, ...releasedCandidates],
      checkpointSummary,
    });
    const priorReport = await json('build/ticket-18-faction-production-v1/'
      + args[4] + '/report.json');
    if (!priorReport.sourceReviewPassed
      || priorReport.failure !== null
      || hash(priorReport.candidateHashes)
        !== hash(releasedCandidates.map(candidate => candidate.hash))) {
      fail('FACTION_COMPLETE_RELEASE_REPORT_INVALID');
    }
    const resumeReceipt = seal({
      version: 'faction_complete_release_resume_v1',
      runId: args[4], recipeHash: releasedRecipe.hash,
      releaseManifestHash: release.hash,
      candidateHashes: releasedCandidates.map(candidate => candidate.hash),
      nextCursor: { nextFaction: null, nextSection: null,
        nextRevision: null, nextRole: null },
      dependencyReconstructionPerformed: false,
      dshPrepared: false, keychainAccessed: false,
      capabilityRenewalPerformed: false, providerCalls: 0,
      sourceRefreshPerformed: false,
      semanticAcceptanceInherited: false, trainingTruth: false,
    });
    const releaseDir = path.join(base, args[4]);
    await writeFile(path.join(releaseDir, 'production-release-v1.json'),
      JSON.stringify(release, null, 2));
    await writeFile(path.join(releaseDir, 'complete-release-resume-v1.json'),
      JSON.stringify(resumeReceipt, null, 2));
    console.log(JSON.stringify({ event: 'stable-production-release',
      release: release.release, runId: args[4],
      existingSkills: release.existingSkills.length,
      checkpoints: checkpointSummary, providerCalls: 0 }));
    for (const candidate of releasedCandidates) {
      console.log(JSON.stringify({ event: 'faction-release-reused',
        faction: candidate.factionRecordKey.split(':')[1],
        hash: candidate.hash, sections: candidate.sections.length,
        providerCalls: 0 }));
    }
    console.log(JSON.stringify({ event: 'report', runId: args[4],
      factionsGenerated: releasedCandidates.length,
      sourceReviewPassed: true, failure: null,
      tokens: priorReport.ledger.knownTokens,
      cumulativeTokens: priorReport.cumulativeKnownTokensLowerBound,
      cumulativeCny: priorReport.cumulativeEstimateOrReserveCny,
      nextCursor: resumeReceipt.nextCursor,
      providerCalls: 0, hash: resumeReceipt.hash }));
    process.exit(0);
  }
}
...
await assertLegacyReadinessCodeHashes(reviewFocusNormalizationReadiness.codeHashes,
  'FACTION_REVIEW_FOCUS_NORMALIZATION_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(structuredReviewReadiness.codeHashes,
  'FACTION_STRUCTURED_REVIEW_CODE_DRIFT');
...
for (const gate of unitRoleRepairGates) {
  ...
  await assertLegacyReadinessCodeHashes(gate.codeHashes,
    'FACTION_UNIT_REPAIR_READINESS_CODE_DRIFT');
}
...
const modelLifecycleSelections = !requestBetaModel
  && parentRecipe.modelLifecycleBinding
  ? parentRecipe.modelLifecycleSelections
  : !requestBetaModel && parentRecipe.executionModelBinding
    ? seal({ version: 'faction_model_selection_pair_v1', bindingHash: modelLifecycleBinding.hash,
    base: selectFactionExecutionModelV1({ selectedBinding: modelLifecycleBinding,
      legacyProfileRef: factionProfileRefV1(profile) }).decision,
    capacity: selectFactionExecutionModelV1({ selectedBinding: modelLifecycleBinding,
      legacyProfileRef: factionProfileRefV1(capacityProfile) }).decision,
    trainingTruth: false }) : null;
...
if (budgetReadiness) {
  ...
  await assertLegacyReadinessCodeHashes(budgetReadiness.codeHashes,
    'FACTION_BUDGET_READINESS_CODE_DRIFT');
}
...
await assertLegacyReadinessCodeHashes(
  [...slotReviewRuntimeProof.codeHashes, ...slotReviewReadiness.codeHashes],
  'FACTION_SLOT_REVIEW_READINESS_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(fieldRecoveryReadiness.codeHashes,
  'FACTION_FIELD_RECOVERY_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(fieldValueReadiness.codeHashes,
  'FACTION_FIELD_VALUE_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(fieldSourceReadiness.codeHashes,
  'FACTION_FIELD_SOURCE_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(mixedReviewReadiness.codeHashes,
  'FACTION_MIXED_REVIEW_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(resumeReadiness.codeHashes,
  'FACTION_RESUME_RELIABILITY_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(contractProjectionReadiness.codeHashes,
  'FACTION_CONTRACT_PROJECTION_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(structuralJsonReadiness.codeHashes,
  'FACTION_STRUCTURAL_JSON_READINESS_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(sourceExpansionReadiness.codeHashes,
  'FACTION_REVIEW_SOURCE_EXPANSION_READINESS_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(
  [...parsedWireReadiness.codeHashes, ...parsedWireDshProof.codeHashes],
  'FACTION_PARSED_WIRE_SCHEMA_READINESS_CODE_DRIFT');
...
const outlineCapacityReadiness = await json('build/ticket-18-faction-production-v1/outline-capacity-readiness-v1.json');
if (!outlineCapacityReadiness.passed || outlineCapacityReadiness.providerCalls !== 0
  || outlineCapacityReadiness.bindingHash !== outlineCapacityBinding.hash)
  fail('FACTION_OUTLINE_CAPACITY_NOT_READY');
await assertLegacyReadinessCodeHashes(outlineCapacityReadiness.codeHashes,
  'FACTION_OUTLINE_CAPACITY_READINESS_CODE_DRIFT');
const itemsEnvelopeReadiness = await json('build/ticket-18-faction-production-v1/native-items-envelope-readiness-v1.json');
if (!itemsEnvelopeReadiness.passed || itemsEnvelopeReadiness.providerCalls !== 0
  || itemsEnvelopeReadiness.bindingHash !== itemsEnvelopeBinding.hash)
  fail('FACTION_NATIVE_ITEMS_ENVELOPE_NOT_READY');
await assertLegacyReadinessCodeHashes(itemsEnvelopeReadiness.codeHashes,
  'FACTION_NATIVE_ITEMS_ENVELOPE_READINESS_CODE_DRIFT');
const modelLifecycleReadiness = modelLifecycleSelections
  ? await json('build/ticket-18-faction-production-v1/model-lifecycle-production-readiness-v1.json')
  : null;
if (modelLifecycleReadiness && (!modelLifecycleReadiness.passed
  || modelLifecycleReadiness.providerCalls !== 0
  || modelLifecycleReadiness.bindingHash !== modelLifecycleBinding.hash
  || modelLifecycleReadiness.selectionPairHash !== modelLifecycleSelections.hash))
  fail('FACTION_MODEL_LIFECYCLE_PRODUCTION_NOT_READY');
...
await assertLegacyReadinessCodeHashes(wireAddressRecoveryReadiness.codeHashes,
  'FACTION_WIRE_ADDRESS_READINESS_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(zergUnitTimingReadiness.codeHashes,
  'FACTION_ZERG_UNIT_READINESS_CODE_DRIFT');
...
for (const gate of [teachRecoveryReadiness, catalogueReviewReadiness, fairnessReadiness, structuredTeachReadiness, reviewRecoveryBudgetReadiness, teachOutputBudgetReadiness, nativeProductionReadiness, teachUncertaintyReadiness, reviewFocusCapacityReadiness, metadataRecoveryReadiness, targetCompletionReadiness, nativeOutputCapacityReadiness, targetIdReviewReadiness, explicitSlotReviewReadiness, wireRuntimeReadiness, reviewDecompositionReadiness, observedSourceRepairReadiness, planningRepairReadiness, proposerAuxiliaryReadiness, draftPolicyReadiness]) {
  ...
  await assertLegacyReadinessCodeHashes(gate.codeHashes,
    'FACTION_CAPACITY_READINESS_CODE_DRIFT');
}
...
for (const gate of sourceCorrectionGates) {
  ...
  await assertLegacyReadinessCodeHashes(gate.codeHashes,
    'FACTION_SOURCE_CORRECTION_READINESS_CODE_DRIFT');
}
...
const main = requestStableRelease
  ? await json('build/ticket-17-production-redesign-v1/readiness.json')
  : await verifyProductionReadiness(root, catalogue);
...
await assertLegacyReadinessCodeHashes(parallelReadiness.codeHashes,
  'FACTION_PARALLEL_CODE_DRIFT');
const gates = [];
...
if (fieldRepairBinding) {
  ...
  await assertLegacyReadinessCodeHashes(fieldRepairReadiness.codeHashes,
    'FACTION_FIELD_SEED_READINESS_CODE_DRIFT');
...
for (const parentRunId of additionalRecoveryRunIds) {
  ...
  await assertLegacyReadinessCodeHashes(gate.codeHashes,
    'FACTION_ADDITIONAL_COMMAND_RECOVERY_CODE_DRIFT');
...
if (phaseFieldReadiness) {
  ...
  await assertLegacyReadinessCodeHashes(phaseFieldReadiness.codeHashes,
    'FACTION_PHASE_SEED_READINESS_CODE_DRIFT');
...
if (useReviewTransaction) {
  ...
  await assertLegacyReadinessCodeHashes(reviewTransactionReadiness.codeHashes,
    'FACTION_REVIEW_TRANSACTION_CODE_DRIFT');
...
await assertLegacyReadinessCodeHashes(executionPolicyReadiness.codeHashes,
  'FACTION_EXECUTION_POLICY_CODE_DRIFT');
for (const row of executionPolicyReadiness.codeHashes)
  if (!files.includes(row.file)) files.push(row.file);
if (requestBetaModel || parentRecipe.executionModelBinding)
  for (const file of FACTION_EXECUTION_MODEL_FILES_V1) if (!files.includes(file)) files.push(file);
for (const file of ['packages/skill-production-v3/faction-model-lifecycle-v1.mjs',
  'scripts/verify-ticket-18-faction-model-fallback-production-v1.mjs'])
  if (modelLifecycleSelections && !files.includes(file)) files.push(file);
...
const next = seal({ version: 'faction_strategy_production_v1', overallRunId: args[2], overallDependencyHash: overallDependency.hash,
  ...
  ...(modelLifecycleSelections ? { modelLifecycleBinding,
    modelLifecycleSelections,
    modelLifecycleReadinessHash: modelLifecycleReadiness.hash } : {}),
  ...
  outlineCapacityBinding, outlineCapacityReadinessHash: outlineCapacityReadiness.hash,
  outlineCapacityOrigin: { runId: outlineCapacityReadiness.originRunId,
    attemptId: outlineCapacityReadiness.originAttemptId },
  itemsEnvelopeBinding, itemsEnvelopeReadinessHash: itemsEnvelopeReadiness.hash,
  itemsEnvelopeOrigin: { runId: itemsEnvelopeReadiness.originRunId,
    attemptId: itemsEnvelopeReadiness.originAttemptId },
...
let productionRelease = null, stableRunLimits = null;
if (requestStableRelease) {
  const existingSkills = [await json(
  ...
  for (const name of ['terran_armed_forces', 'zerg_swarm']) {
    try {
      existingSkills.push(await json('build/ticket-18-faction-production-v1/'
        + args[4] + '/' + name + '-candidate.json'));
    } catch (error) {
      if (error.code !== 'ENOENT') throw error;
    }
  }
  ...
  let checkpointSummary;
  try {
    const registeredRun = releaseDb.prepare(
      'SELECT recipe,cap,calls,token_cap FROM runs WHERE id=?').get(args[4]);
    if (!registeredRun || registeredRun.recipe !== parentRecipe.hash)
      fail('FACTION_PRODUCTION_RELEASE_REGISTERED_RUN_DRIFT');
    stableRunLimits = {
      maxCalls: registeredRun.calls,
      maxCostMicros: registeredRun.cap,
      maxTokens: registeredRun.token_cap,
    };
    const rows = releaseDb.prepare(
      'SELECT state,count(*) AS n FROM steps WHERE run=? GROUP BY state').all(args[4]);
    checkpointSummary = {
      complete: rows.find(row => row.state === 'complete')?.n || 0,
      pending: rows.find(row => row.state === 'pending')?.n || 0,
    };
    const releaseActivity = classifyFactionFieldRecoveryOwnerActivityV1({
      attempts: releaseDb.prepare('SELECT id,state FROM attempts WHERE run=?').all(args[4]),
      steps: releaseDb.prepare('SELECT id,state,expires FROM steps WHERE run=?').all(args[4]),
    });
    if (!releaseActivity.terminal)
      fail('FACTION_PRODUCTION_RELEASE_CHECKPOINT_RUNNING');
  } finally { releaseDb.close(); }
  productionRelease = createFactionProductionReleaseManifestV1({
    runId: args[4], recipe: parentRecipe, currentRecipe: next,
    existingSkills, checkpointSummary,
  });
  console.log(JSON.stringify({ event: 'stable-production-release',
    release: productionRelease.release, runId: args[4],
    existingSkills: productionRelease.existingSkills.length,
    checkpoints: checkpointSummary, providerCalls: 0 }));
}
let continuation = requestStableRelease && parentRecipe.continuation
  ? restoreFactionProductionContinuationV1({
    filename, manifest: parentRecipe.continuation,
  }) : null;
if (args[4] && !requestStableRelease) {
  ...
  continuation = inspectFactionContinuationV2({ filename, parentRunId: args[4], parent, parentReport, next,
    executionPolicyReadiness,
    executionModelReadiness: parent.executionModelBinding
      || next.executionModelBinding ? executionModelReadiness : null,
    ...
    modelLifecycleMigration: modelLifecycleSelections
      && (!parent.modelLifecycleBinding
        || parent.modelLifecycleReadinessHash !== modelLifecycleReadiness.hash)
      ? { readiness: modelLifecycleReadiness } : null,
    outlineCapacityMigration: parent.outlineCapacityBinding ? null
      : { readiness: outlineCapacityReadiness, input: inputs.find(row =>
        row.hash === outlineCapacityReadiness.inputHash) },
    itemsEnvelopeMigration: { readiness: itemsEnvelopeReadiness, inputs },
...
const recipe = requestStableRelease ? parentRecipe
  : continuation ? seal({ ...nextBody, continuation: continuation.manifest }) : next;
const continuationParentRunId = requestStableRelease && continuation
  ? continuation.manifest.parentRunId : args[4];
const continuationParentRecipe = requestStableRelease && continuation
  ? await json('build/ticket-18-faction-production-v1/'
    + continuationParentRunId + '/recipe.json') : parentRecipe;
const recoveryAncestors = requestStableRelease
  ? fieldRecoveryScope.ancestors.filter(row => row.hash !== recipe.hash)
  : fieldRecoveryScope.ancestors;
const outlineCapacityImports = continuation?.outlineCapacityImports || [];
const itemsEnvelopeImports = continuation?.itemsEnvelopeImports || [];
if (outlineCapacityImports.length) {
  const permit = continuation.manifest.outlineCapacityMigration?.permit;
  if (outlineCapacityImports.length !== 1 || !permit
    || permit.inputHash !== outlineCapacityReadiness.inputHash
    || permit.originRunId !== outlineCapacityReadiness.originRunId
    || permit.originAttemptId !== outlineCapacityReadiness.originAttemptId
    || outlineCapacityImports[0].rejected.hash !== permit.rejectedCandidateHash
    || outlineCapacityImports[0].issue.safeReceiptHash
      !== permit.originalFailureReceiptHash) {
    fail('FACTION_OUTLINE_CAPACITY_IMPORT_TAMPERED');
  }
}
if (itemsEnvelopeImports.length) {
  const permits = continuation.manifest.itemsEnvelopeImports || [];
  if (itemsEnvelopeImports.length !== permits.length
    || itemsEnvelopeImports.some(evidence => {
      const permit = permits.find(row =>
        row.originRunId === evidence.attempt.run
        && row.originAttemptId === evidence.attempt.id);
      return !permit || permit.fullRoleId !== evidence.rejected.roleRef.id
        || permit.rejectedCandidateHash !== evidence.rejected.hash
        || permit.originalFailureReceiptHash !== evidence.issue.safeReceiptHash;
    })) fail('FACTION_NATIVE_ITEMS_ENVELOPE_IMPORT_TAMPERED');
}
const volatileStructuralPlans = await Promise.all(inputs.map(input =>
  planFactionVolatileStructuralReissueV1({ root, filename, recipe,
    continuation, input,
    enabled: !!continuation
      && input.hash === itemsEnvelopeReadiness.inputHash })));
const volatileStructuralPlanFor = input => volatileStructuralPlans.find(row =>
  row.inputHash === input.hash);
...
for (const input of inputs.filter(isMixedLane)) {
  const environment = openFactionMixedReviewEnvironmentV1({ filename, recipe,
    parentRunId: continuationParentRunId,
    parentRecipe: continuationParentRecipe,
...
const promptLineage = continuation ? await (await import('../packages/skill-production-v3/faction-prompt-lineage-v1.mjs'))
  .resolveFactionPromptLineageV1({ steps: continuation.steps,
...
if (requestStableRelease && args[0] === '--preflight') {
  console.log(JSON.stringify({ ready: true,
    release: productionRelease.release,
    releaseManifestHash: productionRelease.hash,
    recipeHash: recipe.hash,
    runId: args[4],
    providerCalls: 0,
    compatibility: productionRelease.compatibility,
    existingSkills: productionRelease.existingSkills,
    checkpoints: productionRelease.checkpointSummary,
    blockingFindings: productionRelease.blockingFindings,
  }));
  process.exit(0);
}
if (args[0] === '--preflight') {
  const dryIndex = itemsEnvelopeImports.length
    ? inputs.findIndex(input => input.hash === itemsEnvelopeReadiness.inputHash)
    : outlineCapacityImports.length
      ? inputs.findIndex(input => input.hash === outlineCapacityReadiness.inputHash)
    : inputs.findIndex(input => quarantinesForInput(input).length === 0 || isMixedLane(input));
  if (dryIndex < 0) fail('FACTION_ALL_TASKS_QUARANTINED');
  const dryInput = inputs[dryIndex];
  const dryDsh = outlineCapacityImports.length || itemsEnvelopeImports.length
    ? await prepareDshLoop(root, {
      sessionPolicy: recipe.executionPolicyBinding.sessionPolicy,
    })
    : { binding: gates[2].dshBinding, run: runDirectLoop };
  if (dryDsh.binding.hash !== recipe.dshBindingHash)
    fail('FACTION_DSH_BINDING_DRIFT');
  ...
  const diagnosticDryLocal = Object.freeze({ ...dryLocal,
    acquire(id, input, ttl) {
      ...
      const actualInputHash = hash(input);
      if (!firstInheritedInputDrift && expectedInputHash
        && actualInputHash !== expectedInputHash) {
        firstInheritedInputDrift = {
          id, expectedInputHash, actualInputHash,
  ...
  const inheritedDryStore = continuation
    ? withCheckpointContinuation(diagnosticDryLocal, continuation)
    : diagnosticDryLocal;
  const dryStore = continuation
    ? withFactionVolatileStructuralStoreV1({
      directStore: diagnosticDryLocal, inheritedStore: inheritedDryStore,
      plans: volatileStructuralPlans })
    : inheritedDryStore;
  ...
  try {
    ...
    const dryStructuralRuntime = createFactionStructuralReviewLaneV1({ root, filename, recipe, input: dryInput,
    ...
    const drySlotRuntime = withFactionVolatileStructuralRuntimeV1({
      filename, recipe, input: dryInput,
      executionPolicy: structuredReviewPolicy,
      structuralRuntime: dryStructuralRuntime,
      fallbackRuntime: drySlotBaseRuntime,
      plan: volatileStructuralPlanFor(dryInput),
      onProgress: row => console.log(JSON.stringify({
        event: 'volatile-structural-reissue', ticket: 18, slice: 174,
        faction: dryInput.factionRecordKey.split(':')[1], ...row })) });
    const dryFieldRuntime = createFactionFieldRecoveryLaneV1({ filename, recipe, ancestors: recoveryAncestors,
    ...
    const dryProjectionResolver = createFactionContractProjectionResolverV1({ filename, recipe, ancestors: recoveryAncestors,
    ...
    const dryNativeRuntime = createFactionNativeProductionRuntimeV1({ input: dryInput,
      ...
      store: dryStore, dsh: dryDsh,
      executionPolicy: structuredReviewPolicy, dry: true,
      ...
      proposerAuxiliaryImports: proposerAuxiliaryReadiness.origins
        .filter(row => row.inputHash === dryInput.hash)
        .map(row => readFactionTeachFailureEvidenceV1({ filename,
          runId: row.originRunId, attemptId: row.originAttemptId })),
      readProposerAuxiliaryFailure: ({ prepared, failureReceiptHash }) =>
        readFactionTeachFailureEvidenceV1({ filename, runId: args[4],
          fullRoleId: prepared.fullRoleId, failureReceiptHash }),
      answerCompletionEnabled: true,
      answerCompletionImports: dryInput.hash === targetCompletionReadiness.nativeInputHash
        ? [readFactionTeachFailureEvidenceV1({ filename,
          runId: targetCompletionReadiness.originRunId,
          attemptId: targetCompletionReadiness.nativeOriginAttemptId })] : [],
      referenceSetImports: dryInput.hash === metadataRecoveryReadiness.nativeInputHash
        ? [readFactionTeachFailureEvidenceV1({ filename,
          runId: metadataRecoveryReadiness.originRunId,
          attemptId: metadataRecoveryReadiness.nativeOriginAttemptId })] : [],
      ...
      legacyRoleIds: (continuation?.steps || []).filter(row => !usesFactionNativeProductionInputV3(row.artifact)).map(row => row.id) });
    const dryItemsRuntime = withFactionNativeItemsEnvelopeRecoveryV1({
      input: dryInput, runtime: dryNativeRuntime, store: dryStore,
      dsh: dryDsh, binding: itemsEnvelopeBinding,
      imports: itemsEnvelopeImports.filter(row =>
        row.rejected.roleRef.id.startsWith('faction.'
          + dryInput.factionRecordKey.split(':')[1] + '.')),
      outputCapacityBinding: nativeOutputCapacityBinding,
      frozenRoleIds: nativeOutputCapacityFrozenRoleIds,
      proposerBatchBinding, proposerAuxiliaryCapacityBinding,
      targetReconstructionBinding: nativeTargetReconstructionBinding,
      draftEnvelopeBinding, dry: true,
      onProgress: row => console.log(JSON.stringify({
        event: 'items-envelope', ticket: 18, slice: 174,
        faction: dryInput.factionRecordKey.split(':')[1], ...row })) });
    const dryOutlineRuntime = withFactionOutlineCapacityRecoveryV1({ input: dryInput,
      runtime: dryItemsRuntime, store: dryStore,
      dsh: dryDsh,
      binding: outlineCapacityBinding,
      imports: outlineCapacityImports.filter(row =>
        row.rejected.roleRef.id.startsWith('faction.'
          + dryInput.factionRecordKey.split(':')[1] + '.')),
      baseExecutionPolicy: structuredReviewPolicy,
      outputCapacityBinding: nativeOutputCapacityBinding,
      frozenRoleIds: nativeOutputCapacityFrozenRoleIds,
      proposerBatchBinding, proposerAuxiliaryCapacityBinding,
      targetReconstructionBinding: nativeTargetReconstructionBinding,
      dry: true,
      onProgress: row => console.log(JSON.stringify({ event: 'outline-capacity', ticket: 18,
        slice: 174, faction: dryInput.factionRecordKey.split(':')[1], ...row })) });
    const dryDispatch = { async role(request) {
      try { return await dryOutlineRuntime.role(request); }
      catch (error) {
  ...
  const reusableRoleIds = new Set((continuation?.manifest.reusable || [])
    .map(row => canonicalPromptRoleId(row.id)));
  const volatileQuarantinedRoleIds = new Set(volatileStructuralPlans
    .flatMap(row => row.quarantineRoleIds).map(canonicalPromptRoleId));
  ...
  if (!firstUncachedRole || !fullUncachedRole.startsWith(factionPrefix + '.' + factionPrefix + '.')
    ...
    || reusableRoleIds.has(fullUncachedRole)
      && !volatileQuarantinedRoleIds.has(fullUncachedRole)
  ...
  console.log(JSON.stringify({ ready: true, recipeHash: recipe.hash, providerCalls: 0, factions: inputs.map(i => i.factionRecordKey),
    ...
    newExecutionModel: recipe.modelLifecycleSelections?.base.model
      || recipe.executionModelBinding?.model || profile.model })); process.exit(0);
}
const runId = 'faction-v1-' + recipe.hash.slice(0, 20), out = path.join(base, runId); await mkdir(out, { recursive: true });
const inherited = requestStableRelease
  ? recipe.continuation?.accounting || { calls: 0, tokens: 0, costMicros: 0 }
  : continuation?.manifest.accounting || { calls: 0, tokens: 0, costMicros: 0 };
const runLimits = requestStableRelease ? stableRunLimits : {
  maxCalls: limits.maxCalls - inherited.calls,
...
const localStore = openProductionStore(filename, { runId, recipeHash: recipe.hash,
  ...runLimits, resetCallWindow: requestStableRelease, limitMode: 'alert',
  onLimitAlert: alert => console.log(JSON.stringify({
    event: 'production-soft-limit-alert', ticket: 18, slice: 174, ...alert })) });
...
const inheritedPolicyStore = continuation
  ? withCheckpointContinuation(policyStore, continuation) : policyStore;
const resumablePolicyStore = continuation
  ? withFactionVolatileStructuralStoreV1({ directStore: policyStore,
    inheritedStore: inheritedPolicyStore, plans: volatileStructuralPlans })
  : inheritedPolicyStore;
const parallelControl = createFactionParallelControlV1(resumablePolicyStore,
  { failureRouter });
...
const wallLimitAlerts = new Set();
const observeWallLimit = scope => {
  const budgetClockStart = isFactionBudgetEpochV1(recipe.budgetExtension)
    ? Date.parse(recipe.budgetExtension.startedAt) : began;
  const elapsedMs = Date.now() - budgetClockStart;
  if (elapsedMs < limits.maxWallMs || wallLimitAlerts.has(scope)) return;
  wallLimitAlerts.add(scope);
  console.log(JSON.stringify({ event: 'production-soft-limit-alert', ticket: 18,
    slice: 174, version: 'production_soft_limit_alert_v1', kind: 'wall_time',
    scope, observed: elapsedMs, configuredThreshold: limits.maxWallMs,
    action: 'continue', paymentRequiredStillBlocks: true,
    trainingTruth: false }));
};
const candidates = [];
if (requestStableRelease) {
  for (const [index, input] of inputs.entries()) {
    ...
    if (productionRelease.existingSkills.some(row =>
      row.factionRecordKey === input.factionRecordKey)) {
      candidates[index] = await json('build/ticket-18-faction-production-v1/'
        + args[4] + '/' + name + '-candidate.json');
    }
  }
}
if (requestStableRelease && candidates.filter(Boolean).length === inputs.length) {
  for (const [index, candidate] of candidates.entries()) {
    console.log(JSON.stringify({ event: 'faction-release-reused',
      faction: inputs[index].factionRecordKey.split(':')[1],
      hash: candidate.hash, sections: candidate.sections.length,
      providerCalls: 0 }));
  }
  parallelReceipt = seal({
    version: 'faction_complete_release_resume_v1',
    releaseManifestHash: productionRelease.hash,
    candidateHashes: candidates.map(candidate => candidate.hash),
    nextCursor: { nextFaction: null, nextSection: null,
      nextRevision: null, nextRole: null },
    dshPrepared: false, capabilityRenewalPerformed: false,
    providerCalls: 0, sourceRefreshPerformed: false,
    semanticAcceptanceInherited: false, trainingTruth: false,
  });
  await put('recipe', recipe);
  await put('production-release-v1', productionRelease);
  await put('parallel-execution', parallelReceipt);
  const ledger = store.summary(), global = store.globalSummary();
  const report = seal({
    runId, recipeHash: recipe.hash,
    overallDependencyHash: overallDependency.hash,
    readinessHashes: [structuredGenerationReadiness,
      structuredReviewReadiness, reviewFocusNormalizationReadiness,
      draftPolicyReadiness, ...gates, ...unitRoleRepairGates,
      ...sourceCorrectionGates].map(gate => gate.hash),
    parallelReceipt, quarantinedTasks,
    candidateHashes: candidates.map(candidate => candidate.hash),
    factionsGenerated: candidates.length, sourceReviewPassed: true,
    failure: null, ledger,
    continuation: recipe.continuation || null,
    productionReleaseHash: productionRelease.hash,
    adaptedExistingSkillHashes:
      productionRelease.existingSkills.map(row => row.artifactHash),
    cumulativeKnownTokensLowerBound: historyTokens + global.knownTokens,
    cumulativeEstimateOrReserveCny:
      (historyMicros + global.reservedOrSettledMicros) / 1e6,
    budgetEpoch: factionBudgetEpochProgressV1({
      extension: recipe.budgetExtension, global }),
    ctx2skillLoopUsed: true, harnessLoopUsed: true,
    targetGames: ['starcraft-tmg'],
    roleRoutes: ['Teach', 'Ctx2Skill', 'Challenger', 'Reasoner', 'Judge',
      'Proposer', 'Generator', 'structured_source_reviewer',
      'structured_local_editor'],
    independentEvaluationPerformed: false,
    actualRoomReplayPerformed: false,
    strategyEffectivenessProven: false, formalSkillsAccepted: 0,
    promotions: [], costNotificationReceiptHash: null,
    nextCostNotificationThresholdMicros: 100_000_000,
    sourceRefreshPerformed: false, trainingTruth: false,
    elapsedMs: Date.now() - began,
  });
  await put('report', report);
  console.log(JSON.stringify({ event: 'report', runId,
    factionsGenerated: report.factionsGenerated,
    sourceReviewPassed: report.sourceReviewPassed, failure: null,
    tokens: ledger.knownTokens,
    cumulativeTokens: report.cumulativeKnownTokensLowerBound,
    cumulativeCny: report.cumulativeEstimateOrReserveCny,
...
try {
  ...
  const costProjection = projectFactionCumulativeCostV2({ historyMicros,
  ...
  for (const alert of costProjection.softAlerts) console.log(JSON.stringify({
    event: 'production-soft-limit-alert', ticket: 18, slice: 174,
    version: 'production_soft_limit_alert_v1', ...alert,
  ...
  if (!isFactionBudgetEpochV1(recipe.budgetExtension) && costProjection.projectedMaximumCumulativeMicros >= 100_000_000) {
    try {
      ...
      verifyFactionCostNotificationV1({ projection: costProjection,
        notification: costNotificationReceipt,
        budgetExtension: recipe.budgetExtension });
    } catch (error) {
      console.log(JSON.stringify({ event: 'production-soft-limit-alert',
        ticket: 18, slice: 174, version: 'production_soft_limit_alert_v1',
        kind: 'cost_notification_receipt_unavailable_or_stale',
        code: error.code || 'CNY_NOTIFICATION_CHECK_FAILED',
        currentCumulativeMicros: costProjection.currentCumulativeMicros,
        action: 'continue', paymentRequiredStillBlocks: true,
        trainingTruth: false }));
    }
  }
  observeWallLimit('startup');
  ...
  const lifecycleBase = recipe.modelLifecycleSelections
    ? selectFactionExecutionModelV1({ selectedBinding: recipe.modelLifecycleBinding,
      legacyProfileRef: recipe.modelLifecycleSelections.base.legacyProfileRef,
      priorDecision: recipe.modelLifecycleSelections.base }).profile : null;
  const lifecycleCapacity = recipe.modelLifecycleSelections
    ? selectFactionExecutionModelV1({ selectedBinding: recipe.modelLifecycleBinding,
      legacyProfileRef: recipe.modelLifecycleSelections.capacity.legacyProfileRef,
      priorDecision: recipe.modelLifecycleSelections.capacity }).profile : null;
  const liveProfiles = {base:lifecycleBase || factionExecutionProfileV1({binding:recipe.executionModelBinding || null}),
    capacity:lifecycleCapacity || factionExecutionProfileV1({binding:recipe.executionModelBinding || null,legacyProfileRef:nativeOutputCapacityBinding.profileRef})};
  ...
  const activeExecutionModelBinding = recipe.modelLifecycleSelections
    ? null : recipe.executionModelBinding || null;
  ...
  const beforeNewProviderSend = () => {if(activeExecutionModelBinding) assertFactionExecutionModelNewSendV1(activeExecutionModelBinding);};
  await put('recipe', recipe);
  if (productionRelease) await put('production-release-v1', productionRelease);
  ...
  if (requestStableRelease && candidates[index]) {
    console.log(JSON.stringify({ event: 'faction-release-reused',
      faction: name, hash: candidates[index].hash,
      sections: candidates[index].sections.length, providerCalls: 0 }));
  ...
  const model = createFactionAccountedModelV1({ store, recovery: commandRecovery, additionalRecoveries,
    executionModelBinding:activeExecutionModelBinding,
    ...
    complete: (providerRequest, { signal } = {}) => {
      ...
      observeWallLimit('provider_send');
  ...
  const runtime = bindFactionLaneRuntimeV1(createProductionRuntimeV3({ store,
    ...
    if (isMixedLane(input)) mixedRuntime = createFactionMixedProductionRuntimeV1({ filename, recipe,
      parentRunId: continuationParentRunId,
      parentRecipe: continuationParentRecipe,
      input, diagnosis: reviewDecompositionDiagnosis,
    ...
    const ensureFieldCapability = createFactionFieldValueCapabilityResolverV1({ filename, store,
      allowedRunIds: [runId, ...recoveryAncestors.map(r => 'faction-v1-' + r.hash.slice(0, 20))],
    ...
    const readFieldSourceHandoff = createFactionFieldSourceReaderV1({ filename, recipe, ancestors: recoveryAncestors,
      runId, input, allowCurrentRunning: true });
    const baseCompleteFieldValues = createFactionFieldValueCompleterV1({ filename, recipe, ancestors: recoveryAncestors,
    ...
    const contractProjectionResolver = createFactionContractProjectionResolverV1({ filename, recipe, ancestors: recoveryAncestors,
    ...
    const nativeSlotOptions = { input, runtime: catalogueRuntime, store, dsh, wireRecovery,
      ...
      parsedReviewNarrativeBinding,
      reviewSoftLimitBinding,
      readReviewSoftLimitAttempt: readPriorReviewAttempt,
      ambiguousReplacementBinding: recipe.ambiguousReplacementBinding,
      readReviewArtifact: identity => readFactionReviewArtifactByHashV1({ filename,
    ...
    // A volatile origin is deliberately being replaced by a fresh accounted
    // request. Its fallback must not consult either historical raw/parsed-wire
    // recovery or prior-attempt import before reaching the Provider boundary.
    const freshNativeSlotRuntime = createFactionStructuredReviewRuntimeV1({
      ...nativeSlotOptions,
      parsedWireSchemaBridgeBinding: undefined,
      readParsedWireRecovery: undefined,
      parsedReviewValueBinding: undefined,
      parsedReviewNarrativeBinding: undefined,
      readReviewArtifact: undefined,
      readPriorReviewAttempt: undefined,
    });
    const nativeStructuralRuntime = createFactionStructuralReviewLaneV1({ root, filename, runId, recipe, input,
    ...
    const nativeSlotRuntime = withFactionVolatileStructuralRuntimeV1({
      filename, recipe, input, executionPolicy: structuredReviewPolicy,
      structuralRuntime: nativeStructuralRuntime,
      fallbackRuntime: freshNativeSlotRuntime,
      plan: volatileStructuralPlanFor(input),
      onProgress: row => console.log(JSON.stringify({
        event: 'volatile-structural-reissue', ticket: 18, slice: 174,
        faction: name, ...row })) });
    ...
    const fieldRuntime = createFactionFieldRecoveryLaneV1({ filename, recipe, ancestors: recoveryAncestors,
    ...
    const nativeFactionRuntime = createFactionNativeProductionRuntimeV1({ input,
      executionModelBinding:activeExecutionModelBinding,
    ...
    const itemsRuntime = withFactionNativeItemsEnvelopeRecoveryV1({
      input, runtime: nativeFactionRuntime, store, dsh,
      binding: itemsEnvelopeBinding,
      imports: itemsEnvelopeImports.filter(row =>
        row.rejected.roleRef.id.startsWith('faction.' + name + '.')),
      outputCapacityBinding: nativeOutputCapacityBinding,
      frozenRoleIds: nativeOutputCapacityFrozenRoleIds,
      proposerBatchBinding, proposerAuxiliaryCapacityBinding,
      targetReconstructionBinding: nativeTargetReconstructionBinding,
      ...
      onProgress: row => console.log(JSON.stringify({
        event: 'items-envelope', ticket: 18, slice: 174,
        faction: name, ...row })) });
    const teachRuntime = withFactionOutlineCapacityRecoveryV1({ input,
      runtime: itemsRuntime, store, dsh, binding: outlineCapacityBinding,
      imports: outlineCapacityImports.filter(row =>
        row.rejected.roleRef.id.startsWith('faction.' + name + '.')),
      baseExecutionPolicy: structuredReviewPolicy,
      outputCapacityBinding: nativeOutputCapacityBinding,
      ...
      proposerBatchBinding, proposerAuxiliaryCapacityBinding,
      targetReconstructionBinding: nativeTargetReconstructionBinding,
      readCurrentFailure: ({ prepared, failureReceiptHash }) =>
        readFactionTeachFailureEvidenceV1({ filename, runId,
          fullRoleId: prepared.fullRoleId,
          roleRefHash: prepared.roleRef.hash,
          contextManifestRefHash: prepared.contextManifestRef.hash,
          failureReceiptHash }),
      onProgress: row => console.log(JSON.stringify({ event: 'outline-capacity', ticket: 18,
        slice: 174, faction: name, ...row })) });
  ...
  const report = seal({ runId, recipeHash: recipe.hash, overallDependencyHash: overallDependency.hash,
    ...
    continuation: requestStableRelease ? recipe.continuation || null : continuation?.manifest || null,
    productionReleaseHash: productionRelease?.hash || null,
    adaptedExistingSkillHashes:
      productionRelease?.existingSkills.map(row => row.artifactHash) || [],
    cumulativeKnownTokensLowerBound: historyTokens + global.knownTokens,
