...
import { createOfficialExistingStandardMoveContractClosureRuleSliceV1 } from
...
import {
  ...
  OFFICIAL_STANDARD_MOVE_RELATIONSHIP_SCOPE_ID,
} from "../packages/rule-atoms/official-standard-move-relationship-contract-v1.mjs";
import {
...
const ids = OFFICIAL_STANDARD_MOVE_RELATIONSHIP_NODE_IDS_V1;
const acceptance = [];

assert.equal(graph.graphHash,
  "8c9585d590ca7ea2f98b5734604b1a2c725ba6c37a05535a44417dd04141973b");
assert.equal(graph.nodes.length, 6414);
assert.equal(graph.edges.length, 22469);
...
acceptance.push("all_ten_standard_move_consumed_atoms_are_strict_complete");
...
const scale = queryRuleRelationshipImpactV1(graph, {
  startNodeId: ids.pieces,
  targetNodeIds: [ids.unitScaleTest, ids.replayTest],
...
assert.ok(scale.paths.every((entry) => entry.reached));
assert.ok(geometry.paths.every((entry) => entry.reached));
acceptance.push("unit_scale_and_geometry_material_reach_judge_and_replay_evidence");

const scope = extension.coverageScopes.find((entry) => (
  entry.scopeId === OFFICIAL_STANDARD_MOVE_RELATIONSHIP_SCOPE_ID
));
...
assert.ok(brokenAudit.gaps.requiredEdgeGaps.some((entry) => (
  entry.includes(OFFICIAL_STANDARD_MOVE_RELATIONSHIP_SCOPE_ID)
)));
...
console.log(JSON.stringify({
  schema: "starcraft_tmg_standard_move_relationship_public_contract_verification_v1",
