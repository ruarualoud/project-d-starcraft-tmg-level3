...
const { candidate, evidence, releaseEvidence: candidateReleaseEvidence = null } = await inspectFactionCandidateEvidenceV1({
  root, runId: args[2], input, knownRulePolicy, catalogue, context });
const gates = await Promise.all(['production-replay-readiness', 'consumer-evaluation-readiness', 'unit-role-debt-readiness',
  ...
  'consumer-execution-component-v1'].map(json));
for (const gate of gates) {
  if (!gate.passed) fail('FACTION_CONSUMER_READINESS_FAILED');
  const releaseEvidenceDrift = [];
  for (const c of gate.codeHashes) {
    if (sha256(await readFile(path.join(root, c.file))) === c.hash) continue;
    releaseEvidenceDrift.push(c.file);
  }
  if (releaseEvidenceDrift.length) console.log(JSON.stringify({ event: 'consumer-release-evidence-drift',
    readinessHash: gate.hash, files: releaseEvidenceDrift, action: 'continue', providerCalls: 0 }));
}
...
const limits = { maxCalls: 30, maxCostMicros: 15_000_000, maxTokens: 30_000_000, maxWallMs: 90 * 60 * 1000, maxInputBytes: 1_000_000 };
const recipe = seal({ version: 'faction_consumer_evaluation_run_v4', sourceRunId: args[2], inputHash: input.hash,
  ...
  drillManifestHash: drills.manifest.hash, limits,
  applicationDrillManifestHash: applicationDrills.manifest.hash, applicationRepetitionsPerArm: 3,
  applicationGrouping: 'one_rule_family_per_request', plannedCalls: 20,
...
const consumerReleaseEvidence = seal({ version: 'faction_consumer_release_evidence_v1',
  criticalRecipeHash: recipe.hash, readinessHashes: gates.map(g => g.hash), codeHashes,
...
if (args[0] === '--preflight') {
  console.log(JSON.stringify({ ready: true, recipeHash: recipe.hash, candidateHash: candidate.hash, questions: drills.list(input.factionRecordKey).length,
    applicationQuestions: 22, applicationRepetitionsPerArm: 3, applicationGrouping: recipe.applicationGrouping,
    plannedCalls: recipe.plannedCalls, actualProviderCalls: 0, limits })); process.exit(0);
}
...
const store = openProductionStore(filename, { runId, recipeHash: recipe.hash,
  ...limits, resetCallWindow: true, limitMode: 'alert', onLimitAlert: alert =>
    console.log(JSON.stringify({ event: 'consumer-soft-limit-alert',
      ticket: 18, slice: 174, ...alert })) });
...
const emittedOperationalAlerts = new Set();
const emitOperationalAlert = (kind, observed, configuredThreshold) => {
  if (emittedOperationalAlerts.has(kind)) return;
  emittedOperationalAlerts.add(kind);
  console.log(JSON.stringify({ event: 'consumer-soft-limit-alert',
...
const observeWallTime = () => {
  const elapsed = Date.now() - began;
  if (elapsed >= limits.maxWallMs)
    emitOperationalAlert('wall_time', elapsed, limits.maxWallMs);
};
try {
  ...
  if (budgetEpoch) {
    ...
    if (epochProgress.estimateOrReservedMicros + Math.max(0, limits.maxCostMicros - store.summary().reservedOrSettledMicros)
      > budgetEpoch.allowance.costMicros) {
      emitOperationalAlert('budget_epoch_cost',
        epochProgress.estimateOrReservedMicros,
        budgetEpoch.allowance.costMicros);
    }
  } else {
    try {
      checkProductionCostNotificationV1({ cumulativeMicros:
        historyMicros + global.reservedOrSettledMicros,
      additionalMicros: Math.max(0, limits.maxCostMicros
        - store.summary().reservedOrSettledMicros),
      notification: costNotification });
    } catch (error) {
      if (error.code !== 'CNY_100_NOTIFICATION_REQUIRED') throw error;
      emitOperationalAlert('cost_notification',
        historyMicros + global.reservedOrSettledMicros,
        100_000_000);
    }
  }
  ...
  await put('consumer-release-evidence-v1', consumerReleaseEvidence);
  if (candidateReleaseEvidence) await put('candidate-producer-release-evidence-v1', candidateReleaseEvidence);
  ...
  applicationResult = await evaluateFactionRuleUseGroupedV2({ input, candidate, knownRulePolicy,
    drills: applicationDrills, store, model, finalGeneral,
    onProgress: row => console.log(JSON.stringify({ event: 'rule-consumer-v4', ticket: 18, slice: 174, ...row })) });
  ...
  const report = seal({ runId, recipeHash: recipe.hash, resultHash: result?.hash || null, failure, ledger,
    ...
    ruleApplicationSummary: applicationResult?.summary || [], groupedRuleCalls: applicationResult?.calls || 0,
    applicationGrouping: recipe.applicationGrouping, independentlyHeldOutApplicationCases: 0,
