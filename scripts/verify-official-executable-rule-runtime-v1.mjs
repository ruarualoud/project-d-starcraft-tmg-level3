...
const historicalElevationEffectiveSizeRulesSliceReport = JSON.parse(await readFile(
  path.join(
    OUTPUT_DIR,
...
const historicalSpecialTerrainRulesSliceReport = JSON.parse(await readFile(
  path.join(
    OUTPUT_DIR,
...
const historicalModelBaseGeometryRulesSliceReport = JSON.parse(await readFile(
  path.join(
    OUTPUT_DIR,
...
const historicalKeywordSpecialAbilityRulesSliceReport = JSON.parse(await readFile(
  path.join(
    OUTPUT_DIR,
...
const historicalAbilityTimingPriorityRulesSliceReport = JSON.parse(await readFile(
  path.join(
    OUTPUT_DIR,
...
const historicalCardBuildPaymentRulesSliceReport = JSON.parse(await readFile(
  path.join(OUTPUT_DIR,
    "official-card-build-payment-rules-rule-slice-v1-report.json"), "utf8"));
const historicalUnitCardSupplyRulesSliceReport = JSON.parse(await readFile(
  path.join(OUTPUT_DIR,
...
const historicalReserveLifecycleRulesSliceReport = JSON.parse(await readFile(
  path.join(OUTPUT_DIR,
...
const historicalSummonRulesSliceReport = JSON.parse(await readFile(
  path.join(OUTPUT_DIR,
    "official-summon-rules-rule-slice-v1-report.json"), "utf8"));
const historicalRespawnMorphRulesSliceReport = JSON.parse(await readFile(
  path.join(OUTPUT_DIR,
...
const historicalElevationEffectiveSizeRulesSlice =
...
const historicalKeywordSpecialAbilityRulesSlice =
  historicalKeywordSpecialAbilityRulesSliceReport.slice;
const historicalAbilityTimingPriorityRulesSlice =
  historicalAbilityTimingPriorityRulesSliceReport.slice;
const historicalCardBuildPaymentRulesSlice =
  historicalCardBuildPaymentRulesSliceReport.slice;
const historicalUnitCardSupplyRulesSlice =
...
const historicalReserveLifecycleRulesSlice =
...
const historicalSummonRulesSlice = historicalSummonRulesSliceReport.slice;
...
assert.equal(historicalRespawnMorphRulesSlice.previousSliceHash,
  historicalSummonRulesSlice.sliceHash);
assert.equal(historicalSummonRulesSlice.previousSliceHash,
...
assert.equal(historicalReserveLifecycleRulesSlice.previousSliceHash,
...
assert.equal(historicalUnitCardSupplyRulesSlice.previousSliceHash,
  historicalCardBuildPaymentRulesSlice.sliceHash);
assert.equal(historicalCardBuildPaymentRulesSlice.previousSliceHash,
  historicalAbilityTimingPriorityRulesSlice.sliceHash);
assert.equal(historicalAbilityTimingPriorityRulesSlice.previousSliceHash,
  historicalKeywordSpecialAbilityRulesSlice.sliceHash);
assert.equal(historicalKeywordSpecialAbilityRulesSlice.previousSliceHash,
...
assert.equal(historicalSpecialTerrainRulesSlice.previousSliceHash,
  historicalElevationEffectiveSizeRulesSlice.sliceHash);
assert.equal(historicalElevationEffectiveSizeRulesSlice.previousSliceHash,
...
await check("runtime_binds_the_exact_cumulative_catalogue_and_known_executors", () => {
  ...
  assert.equal(rulesRuntime.descriptor.executableRuleAtomCount, 793);
  assert.equal(rulesRuntime.descriptor.nonExecutableRuleAtomCount, 233);
  ...
  assert.equal(
    ...
    "cdf4d39c1bc6eac0a2d1d5703ff856b059a172a834563656b1a4784d0547e97e",
  );
  assert.equal(
  ...
  assert.deepEqual(
    manifestById.get("authority.special-terrain-rules-v1").actionTypes,
    ["resolve_special_terrain_rules_procedure"],
  );
  assert.deepEqual(
    manifestById.get("authority.model-base-geometry-rules-v1").actionTypes,
  ...
  assert.deepEqual(
    manifestById.get("authority.ability-timing-priority-rules-v1").actionTypes,
    ["resolve_ability_timing_priority_rules_procedure"],
  );
  assert.deepEqual(
    manifestById.get("authority.card-build-payment-rules-v1").actionTypes,
    ["resolve_card_build_payment_rules_procedure"],
  );
  assert.deepEqual(
    manifestById.get("authority.unit-card-supply-rules-v1").actionTypes,
    ["resolve_unit_card_supply_rules_procedure"],
  );
  assert.deepEqual(
    manifestById.get("authority.round-phase-activation-rules-v1").actionTypes,
...
const report = {
  ...
  ctx2skill: {
    ...
    promotions: [],
    blocks: [
      "remaining_119_actionable_rule_atoms_not_executable",
