...
for (const r of [recipe, probeRecipe, gate]) for (const row of r.codeHashes) {
  if (sha256(await readFile(path.join(ROOT, row.file))) !== row.hash) fail('STRATEGY_CONTINUATION_CODE_DRIFT', { file: row.file });
}
if (recipe.inputHash !== input.hash || recipe.caseCorpusHash !== corpus.hash || !gate.passed
...
const amendment = seal({ schema: 'ticket18_general_strategy_opening_fence_amendment_v1',
  ...
  codeHashes: await codeHashes(['scripts/run-ticket-18-general-strategy-continuation-v1.mjs']),
...
const out = path.join(BUILD, PARENT_RUN, 'opening-fence-continuation-v1');
...
const execution = baseStore.acquire('opening-fence-execution-amendment-v1', { amendmentHash: amendment.hash });
...
try {
  ...
  const adapter = withOpeningFenceRecoveryV1({ adapter: port.adapter,
    readWire: id => json(prefix + PARENT_RUN + '/opening-fence-continuation-v1/wire/' + id + '.json') });
