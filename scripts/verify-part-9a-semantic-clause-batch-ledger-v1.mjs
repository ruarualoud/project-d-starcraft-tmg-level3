...
check("part_9a_is_pinned_to_current_official_evidence", () => {
  assert.equal(ledger.sourcePart, "9");
  assert.equal(ledger.batchId, "part-9a");
...
check("all_32_batch_candidates_are_classified_once", () => {
  assert.equal(audit.counts.sourceAnchors, 6);
  assert.equal(audit.counts.sourceCandidates, 32);
  assert.equal(audit.counts.classifiedCandidates, 32);
  assert.equal(audit.counts.unclassifiedCandidates, 0);
  assert.equal(audit.counts.duplicateCandidateAssignments, 0);
});

check("part_9a_has_24_reviewed_semantic_boundaries", () => {
  assert.equal(audit.counts.canonicalClauses, 24);
...
check("faction_tags_scale_table_and_slots_are_grouped", () => {
  const byId = new Map(ledger.canonicalClauses.map((clause) => [clause.clauseId, clause]));
  assert.deepEqual(byId.get("core:9.1.2:any-tag-mismatch")?.candidateOrdinals, [6, 7]);
  assert.deepEqual(byId.get("core:9.1.2:fewer-tags-eligible")?.candidateOrdinals, [8, 9]);
  assert.equal(byId.get("core:9.1.4:engagement-scale-table")?.disposition, "review_required");
  assert.deepEqual(byId.get("core:9.1.5:tactical-card-slot-purchase")?.candidateOrdinals, [1, 2]);
});
...
check("gaps_dependency_drift_and_incomplete_merge_fail_closed", () => {
  const gap = structuredClone(OFFICIAL_PART_9A_SEMANTIC_REVIEW_BINDING_V1);
  ...
  const drift = structuredClone(OFFICIAL_PART_9A_SEMANTIC_REVIEW_BINDING_V1);
...
check("one_reviewed_batch_grants_no_global_rule_skill_or_training_authority", () => {
  assert.deepEqual(mergeStatus.reviewedBatchIds, ["part-9a"]);
  assert.deepEqual(mergeStatus.pendingBatchIds, ["part-9b", "part-9c"]);
  assert.equal(mergeStatus.counts.reviewedCandidates, 32);
  assert.equal(mergeStatus.counts.pendingCandidates, 124);
  assert.equal(mergeStatus.counts.reviewedCanonicalClauses, 24);
...
const report = {
  schema: "starcraft_tmg_part_9a_semantic_clause_batch_ledger_verification_v1",
...
await writeFile(
  path.join(OUTPUT_DIR, "part-9a-semantic-clause-batch-ledger-report.json"),
