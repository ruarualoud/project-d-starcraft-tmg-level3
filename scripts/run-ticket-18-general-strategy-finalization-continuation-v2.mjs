...
import { inspectGeneralFinalizationSchemaRepairContinuationV1,
  withGeneralFinalizationSchemaRepairContinuationV1 } from '../packages/strategy-skills/general-finalization-continuation-v1.mjs';
...
if (process.argv.length !== 3 || !['--preflight', '--live'].includes(mode)) fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_ARGUMENTS_INVALID');
await verifyPreviousReadiness();
const SCHEMA_PARENT_RUN = 'general-final-resume-514577883760a63fbc70';
...
const schemaParentRelative = runPrefix + 'general-strategy-final-v1/' + SCHEMA_PARENT_RUN + '/';
const [previousInput, originalRecipe, capabilities, calibration, separation, opponentReport,
  ...
  json(schemaParentRelative + 'recipe.json'), json(schemaParentRelative + 'report.json'),
...
if (!calibration.calibrationPassed || calibration.failure || opponentReport.failure
  || separation.initial.policies.length !== 7 || parentReport.sourceAxesAccepted.length !== 7
  || parentReport.sourceFieldsAccepted !== 77 || parentReport.caseResults !== 0
  || parentReport.failure?.code !== 'STRUCTURED_PROVIDER_SCHEMA_INVALID') {
  fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_PREREQUISITE_INVALID');
}
...
if (sourceCandidates.some(candidate => candidate.status !== 'model_review_clear_pending_independent_validation')
  ...
  fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_SOURCE_DRIFT');
}
const candidates = sourceCandidates.map(sourceCandidate => prepareFinalGeneralPolicyV1({ input, previousInput, sourceCandidate }));
const failedCase = corpus.cases[0];
const failedAxis = failedCase.prompt.policyAxes[0];
const continuation = inspectGeneralFinalizationSchemaRepairContinuationV1({ filename: DB_PATH,
  parentRunId: SCHEMA_PARENT_RUN, parentRecipe, parentReport, input, compiled: failedCase,
  policy: candidates.find(row => row.axis === failedAxis).policy, expectedInheritedReviews: 21 });
...
for (const receipt of [decisionCapability, calibration.capability]) {
  if (!receipt || !verifyStarcraftTmgProviderCapabilityCurrentV1({ receipt,
    ...
    fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_CAPABILITY_EXPIRED');
  }
}
if (decisionCapability.outputContractRef.hash !== decisionRef.hash) fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_DECISION_CAPABILITY_DRIFT');

const decisionCalls = corpus.cases.reduce((count, row) => count + row.prompt.policyAxes.length, 0);
const expectedNewDecisionCalls = decisionCalls - 1;
const limits = { maxCalls: 20, maxTokens: 9_000_000, maxCostMicros: 6_000_000, maxWallMs: 6 * 60 * 60 * 1000 };
const recipe = seal({ schema: 'ticket18_general_strategy_finalization_schema_continuation_recipe_v1',
  ticket: 18, slice: 174, parentRunId: SCHEMA_PARENT_RUN, parentRecipeHash: parentRecipe.hash,
  ...
  sourceCandidateHashes: sourceCandidates.map(row => row.hash), candidateHashes: candidates.map(row => row.hash),
  sourceRefreshPerformed: false, inheritedReviewCalls: continuation.manifest.inheritedReviews,
  locallyRepairedDecisionCalls: 1, expectedNewDecisionCalls,
  expectedMaximumNewProviderCalls: expectedNewDecisionCalls,
  localRepairPolicy: 'only_missing_selected_candidate_comparison_then_rules_case_regrade',
  futureIdenticalShapeRecovery: 'bounded_local_structural_completion_then_rules_case_regrade',
  ...
  codeHashes: await codeHashes([
    ...
    'scripts/run-ticket-18-general-strategy-finalization-continuation-v2.mjs',
  ]), runtimeAccepted: false, trainingTruth: false });
const before = ledgerSnapshot(); assertLedgerReady(before, limits.maxCostMicros);
const runId = 'general-final-shape-' + recipe.hash.slice(0, 20);
if (mode === '--preflight') {
  console.log(JSON.stringify({ ready: true, ticket: 18, slice: 174, runId,
    ...
    locallyRepairedDecisionCalls: 1, newDecisionCalls: expectedNewDecisionCalls,
    maximumNewProviderCalls: recipe.expectedMaximumNewProviderCalls,
    providerRegenerationCallsForKnownFailure: continuation.localStep.repair.providerRegenerationCalls,
...
const startLease = baseStore.acquire('general-finalization-schema-continuation-start-v1', { recipeHash: recipe.hash });
...
const resumedStore = withGeneralFinalizationSchemaRepairContinuationV1(baseStore, continuation);
...
const journal = { ...resumedStore, reserve(id, request, reserveMicros, reserveTokens) {
  ...
  if (!summary.attempts.some(attempt => attempt.id === id)) {
    ...
    if (calls >= limits.maxCalls || cost + reserveMicros > limits.maxCostMicros
      || tokens + reserveTokens > limits.maxTokens) fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_BUDGET_EXHAUSTED');
...
const sourceAudits = [], caseResults = [], shapeRecoveries = [];
try {
  const dsh = await prepareDshLoop(ROOT);
  if (hash(dsh.binding) !== hash(originalRecipe.dshBinding)) fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_DSH_DRIFT');
  ...
  if (inheritanceEvidence.usedInheritedReviews !== 21 || !inheritanceEvidence.localRepairUsed) {
    fail('GENERAL_FINALIZATION_SCHEMA_CONTINUATION_INHERITANCE_INCOMPLETE');
  }
  await save('shape-recoveries', seal({ schema: 'strategy_decision_shape_recoveries_v1',
    recoveries: shapeRecoveries, providerRegenerationCalls: 0, runtimeAccepted: false, trainingTruth: false }));
...
const report = seal({ schema: 'ticket18_general_strategy_finalization_schema_continuation_report_v1',
  ticket: 18, slice: 174, runId, recipeHash: recipe.hash, parentRunId: SCHEMA_PARENT_RUN,
  parentReportHash: parentReport.hash, continuationManifestHash: continuation.manifest.hash,
...
console.log(JSON.stringify({ event: 'report', completed: report.formalGeneralSkillCompleted,
  ...
  inheritedReviews: report.inheritedReviewCalls, cases: report.caseResults,
