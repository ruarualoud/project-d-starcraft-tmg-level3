...
import {
  createOfficialExistingStandardMoveContractClosureRuleSliceV1,
  verifyOfficialExistingStandardMoveContractClosureRuleSliceV1,
} from
  "../packages/rule-atoms/official-existing-standard-move-contract-closure-rule-slice-v1.mjs";
...
import { createOfficialStandardMoveRelationshipExtensionV1 } from
  "../packages/rule-atoms/official-standard-move-relationship-contract-v1.mjs";
import { OFFICIAL_START_OF_ROUND_V2_ACTION_TYPE } from
...
const OCCURRED_AT = "2026-08-28T23:30:00.000Z";
const SLICE_HASH =
  ...
  return {
    ...
    pieces: [
      ...
      marine(profile, {
        id: "p2-live",
        sideKey: "player2",
        positions: [{ xInches: 40, yInches: 28 }],
      }),
    ],
    startOfRoundHistory: [],
    gameOver: false,
...
function credentials(engine, envelope, sideKey, suffix, input = {}) {
  const authority = engine.issueSeatAuthority({
    grantId: `standard-move-v2-${suffix}-grant`,
    roomId: envelope.roomId,
    matchBindingHash: envelope.matchBindingHash,
    seatKey: sideKey,
    roleMode: input.roleMode || "player",
    principalType: input.principalType || "human",
    capabilities: ["read_legal_space", "preview", "confirm", "apply"],
  });
  const lease = engine.issueControlLease({
    seatAuthority: authority,
    sessionId: `standard-move-v2-${suffix}-session`,
    leaseFence: 1,
...
function applyFinite(engine, envelope, sideKey, predicate, suffix) {
  ...
  const applied = engine.apply({
    ...
    idempotencyKey: `standard-move-v2-${suffix}`,
  });
  assert.equal(applied.ok, true, JSON.stringify(applied));
  return { access, legal, finite, preview, confirmed, applied };
}

function moveDomain(engine, envelope, access, pieceId) {
  const legal = engine.legalSpace(envelope, { seatAuthority: access.authority });
  const domain = legal.parameterDomains.find((entry) => (
    entry.parameterKind === OFFICIAL_STANDARD_MOVE_V2_PARAMETER_KIND
...
const slice = createOfficialExistingStandardMoveContractClosureRuleSliceV1({
  previousSlice: previousReport.slice,
});
const sliceAudit = verifyOfficialExistingStandardMoveContractClosureRuleSliceV1({
  previousSlice: previousReport.slice,
  slice,
});
...
assert.equal(sliceAudit.counts.changedAtoms, 10);
...
assert.ok(OFFICIAL_STANDARD_MOVE_V2_EXECUTOR_ATOM_IDS.every((atomId) => (
  coverage.strictCompleteAtomIds.includes(atomId)
)));
acceptance.push("slice_rebinds_only_ten_existing_atoms_and_closes_one_contract");
...
assert.equal(contentHash(await readFile(path.join(
  ROOT,
  "packages/rule-atoms/official-standard-move-executor-v2.mjs",
...
assert.equal(runtime.descriptor.executorManifest.some((entry) => (
  entry.executorId === OFFICIAL_STANDARD_MOVE_V2_EXECUTOR_ID
    && entry.executorVersion === OFFICIAL_STANDARD_MOVE_V2_EXECUTOR_VERSION
)), true);
...
acceptance.push("current_official_unit_scale_speed_base_and_gauntlet_are_hash_bound");
...
function authoritativeEngine(hmacSecret) {
  return createStarcraftTmgAuthoritativeEngine({
    ...
    now: () => OCCURRED_AT,
    cryptoOptions: {
      keyId: "ticket-11-standard-move-v2-referee-v1",
      privateKey,
      publicKey,
      hmacSecret,
    },
...
const engine = authoritativeEngine("ticket-11-standard-move-v2-seal-v1");
const initial = engine.createEnvelope({
  roomId: "official-standard-move-v2-room",
  dataVersion:
...
acceptance.push("authority_start_v2_and_phase_v1_handoffs_open_current_move_domain");

const access = credentials(engine, phase.applied.envelope, "player1", "move");
const { domain } = moveDomain(
  engine,
...
assert.equal(domain.parameterSchema.exactRemainingPlacementCount, 3);
acceptance.push("four_model_unit_exposes_multi_model_speed_and_exact_placement_denominator");
...
const preview = engine.preview({
  envelope: phase.applied.envelope,
  seatAuthority: access.authority,
...
const moved = engine.apply({
  envelope: phase.applied.envelope,
  expectedStateRevision: phase.applied.envelope.stateRevision,
  preview: preview.preview,
  confirmation: confirmed.confirmation,
  seatAuthority: access.authority,
  controlLease: access.lease,
  idempotencyKey: "standard-move-v2-valid",
});
assert.equal(moved.ok, true, JSON.stringify(moved));
assert.equal(moved.envelope.state.pieces[0].models[0].xInches, 14);
assert.equal(moved.envelope.state.pieces[0].activatedPhases.movement, true);
assert.equal(moved.envelope.state.activeSideKey, "player2");
assert.equal(moved.receipt.refereeSignature.signatureAlgorithm, "ed25519");
acceptance.push("authority_move_applies_geometry_activation_and_alternation_atomically");
...
for (const key of [
  ...
  assert.deepEqual(moved.envelope.state[key], phase.applied.envelope.state[key], key);
}
acceptance.push("move_preserves_scores_sources_setup_history_board_and_cards");

const opponent = credentials(engine, phase.applied.envelope, "player1", "opponent", {
  roleMode: "opponent",
  principalType: "model",
});
...
acceptance.push("opponent_role_can_preview_but_cannot_confirm_or_apply");

const forgedLineageState = clone(phase.applied.envelope.state);
forgedLineageState.log.push({
  id: `log-${forgedLineageState.log.length + 1}`,
  round: 3,
  phase: "movement",
  action: {
    actionType: "move",
  ...
  events: [{
    type: "forged_supply_mutation",
    roundSupplyStateHashBefore:
      forgedLineageState.officialRoundSupplyState.roundSupplyStateHash,
    roundSupplyStateHashAfter: "f".repeat(64),
  }],
});
const forgedEnumeration = runtime.enumerate(forgedLineageState, {
  sideKey: "player1",
  includeDisabled: true,
  matchBinding: phase.applied.envelope.matchBinding,
});
assert.equal(forgedEnumeration.parameterDomains.some((entry) => (
  entry.parameterKind === OFFICIAL_STANDARD_MOVE_V2_PARAMETER_KIND
)), false);
assert.ok(forgedEnumeration.candidates.some((entry) => (
  entry.disabledReason === "STANDARD_MOVE_V2_SUPPLY_LINEAGE_INVALID"
)));
acceptance.push("forged_intermediate_supply_lineage_fails_closed");

const replayEngine = authoritativeEngine("ticket-11-standard-move-v2-rotated-seal-v2");
registerReplayDependencies(replayEngine, initial, snapshot, gameplayDataBundle, runtime);
const journal = [start.applied.receipt, phase.applied.receipt, moved.receipt];
const replayed = replayEngine.replay({ initialEnvelope: initial, journal });
assert.equal(replayed.ok, true, JSON.stringify(replayed));
assert.equal(replayed.envelope.stateHash, moved.envelope.stateHash);
const tamperedJournal = clone(journal);
tamperedJournal[2].events.push({ type: "forged_standard_move_v2" });
...
acceptance.push("three_receipt_ed25519_replay_survives_hmac_rotation_and_rejects_tamper");
...
const report = {
  schema: "starcraft_tmg_existing_standard_move_contract_closure_verification_v1",
  ...
  authorityFixture: {
    ...
    receiptSignatureAlgorithm: moved.receipt.refereeSignature.signatureAlgorithm,
  },
  ...
  rulesTruth: "standard_move_v2_current_handoffs_scale_geometry_with_frozen_v1_isolation",
  trainingTruth: false,
};
