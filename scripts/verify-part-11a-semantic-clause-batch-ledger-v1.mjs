...
check("part_11a_is_pinned_to_current_official_evidence", () => {
  assert.equal(ledger.sourcePart, "11");
  assert.equal(ledger.batchId, "part-11a");
...
check("all_51_batch_candidates_are_classified_once", () => {
  assert.equal(audit.counts.sourceAnchors, 12);
  assert.equal(audit.counts.sourceCandidates, 51);
  assert.equal(audit.counts.classifiedCandidates, 51);
  assert.equal(audit.counts.unclassifiedCandidates, 0);
  assert.equal(audit.counts.duplicateCandidateAssignments, 0);
});

check("part_11a_has_44_reviewed_semantic_boundaries", () => {
  assert.equal(audit.counts.canonicalClauses, 44);
...
check("buff_burrowed_and_combat_tag_boundaries_are_grouped", () => {
  const byId = new Map(ledger.canonicalClauses.map((clause) => [clause.clauseId, clause]));
  assert.deepEqual(byId.get("core:11:buff-target-number")?.candidateOrdinals, [2, 3]);
  assert.deepEqual(byId.get("core:11:buff-value")?.candidateOrdinals, [4, 5]);
  assert.deepEqual(byId.get("core:11:burrowed-engaged-combat-sequence")?.candidateOrdinals,
    [13, 14, 15, 16]);
  assert.deepEqual(byId.get("core:11:ground-tag-elevation-distinction")?.candidateOrdinals,
    [6, 7, 8]);
  assert.equal(byId.get("core:11:available-supply-casualty-final-round")?.disposition,
    "review_required");
});
...
check("gaps_dependency_drift_and_incomplete_merge_fail_closed", () => {
  const gap = structuredClone(OFFICIAL_PART_11A_SEMANTIC_REVIEW_BINDING_V1);
  ...
  const drift = structuredClone(OFFICIAL_PART_11A_SEMANTIC_REVIEW_BINDING_V1);
...
check("one_reviewed_batch_grants_no_global_rule_skill_or_training_authority", () => {
  assert.deepEqual(mergeStatus.reviewedBatchIds, ["part-11a"]);
  assert.deepEqual(mergeStatus.pendingBatchIds,
    ["part-11b", "part-11c", "part-11d", "part-11e", "part-11f"]);
  assert.equal(mergeStatus.counts.reviewedCandidates, 51);
  assert.equal(mergeStatus.counts.pendingCandidates, 300);
  assert.equal(mergeStatus.counts.reviewedCanonicalClauses, 44);
...
const report = {
  schema: "starcraft_tmg_part_11a_semantic_clause_batch_ledger_verification_v1",
...
await writeFile(
  path.join(OUTPUT_DIR, "part-11a-semantic-clause-batch-ledger-report.json"),
