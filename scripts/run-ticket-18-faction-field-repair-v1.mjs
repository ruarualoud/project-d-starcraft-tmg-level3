...
import { createFactionFieldRepairPlanV1, repairFactionSectionFieldsV1 } from '../packages/skill-production-v3/faction-field-repair-v1.mjs';
...
if (args.length !== 3 || !['--preflight', '--live'].includes(args[0]) || args[1] !== '--source-run'
  || !/^faction-v1-[a-f0-9]{20}$/.test(args[2])) fail('FACTION_FIELD_RUN_ARGUMENTS_INVALID');
...
if (args[2] !== 'faction-v1-' + parent.hash.slice(0, 20) || parentReport.recipeHash !== parent.hash
  ...
  || parent.modelHash !== profile.integrity.hash) fail('FACTION_FIELD_RUN_SOURCE_DRIFT');
let sectionResult;
...
try {
  ...
  if (db.prepare('SELECT recipe FROM runs WHERE id=?').get(args[2])?.recipe !== parent.hash
    || db.prepare("SELECT count(*) n FROM steps WHERE run=? AND state='running'").get(args[2]).n) fail('FACTION_FIELD_RUN_PARENT_NOT_TERMINAL');
  const row = db.prepare("SELECT artifact FROM steps WHERE run=? AND id=? AND state='complete'")
    .get(args[2], 'faction.terran_armed_forces.army_resources.1.result');
  if (!row) fail('FACTION_FIELD_RUN_SECTION_MISSING'); sectionResult = verifySeal(verifySeal(JSON.parse(row.artifact)).value);
} finally { db.close(); }
...
if (!inspection.offlineGenerationQualified || inspection.dependencyHash !== parent.overallDependencyHash
  || inspection.receiptHash !== parent.qualificationReceiptHash) fail('FACTION_FIELD_OVERALL_DRIFT');
const catalogue = await loadFrozenSkillEvidence(root), context = createGlobalProductionContext(catalogue);
if (catalogue.hash !== input.catalogueHash || context.hash !== parent.contextHash) fail('FACTION_FIELD_RUN_CONTEXT_DRIFT');
const main = await verifyProductionReadiness(root, catalogue), gates = [];
for (const name of ['field-repair-readiness', 'field-repair-dsh-readiness']) {
  const gate = await json(name);
  if (!gate.passed || gate.inputHash !== input.hash) fail('FACTION_FIELD_READINESS_FAILED');
  for (const c of gate.codeHashes) if (sha256(await readFile(path.join(root, c.file))) !== c.hash) fail('FACTION_FIELD_READINESS_CODE_DRIFT');
  gates.push(gate);
}
if (gates[1].sectionResultHash !== sectionResult.hash || gates[0].actualSectionResultHash !== sectionResult.hash) fail('FACTION_FIELD_READINESS_SECTION_DRIFT');
...
const files = ['packages/skill-production-v3/faction-field-repair-v1.mjs', 'packages/skill-evaluation/faction-semantic-debt-v1.mjs',
  ...
  'packages/skill-production/store.mjs', 'scripts/run-ticket-18-faction-field-repair-v1.mjs'];
...
const recipe = seal({ version: 'faction_field_repair_run_v1', sourceRunId: args[2], sourceRecipeHash: parent.hash,
  sectionResultHash: sectionResult.hash, inputHash: input.hash, planHash: plan.hash, knownRulePolicyHash: knownRulePolicy.hash,
  ...
  mainReadinessHash: main.hash, readinessHashes: gates.map(g => g.hash), dshBindingHash: gates[1].dshBinding.hash,
...
const runId = 'field-repair-' + recipe.hash.slice(0, 20), out = path.join(base, runId); await mkdir(out, { recursive: true });
...
try {
  ...
  if (Date.now() - began >= limits.maxWallMs) fail('FACTION_FIELD_RUN_WALL_EXHAUSTED');
  await put('recipe', recipe); await put('source-section', sectionResult); await put('plan', plan);
  const dsh = await prepareDshLoop(root); if (dsh.binding.hash !== recipe.dshBindingHash) fail('FACTION_FIELD_DSH_DRIFT');
  ...
  candidate = await repairFactionSectionFieldsV1({ input, sectionResult, knownRulePolicy, runtime, store });
  await put('candidate', candidate);
} catch (error) { failure = { code: /^[A-Z0-9_]{3,100}$/.test(error.code || '') ? error.code : 'FACTION_FIELD_RUN_FAILURE', diagnosticHash: hash(String(error.message)) }; }
finally {
