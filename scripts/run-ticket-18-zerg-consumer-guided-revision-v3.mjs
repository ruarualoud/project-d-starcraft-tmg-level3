...
if (args.length !== 3 || !['--preflight', '--live'].includes(args[0])
  || args[1] !== '--parent-consumer-run' || !/^faction-consumer-[a-f0-9]{20}$/u.test(args[2]))
  fail('FACTION_ZERG_GUIDED_REVISION_RUN_ARGUMENTS');
const parentRunId = args[2];
...
try {
  ...
  if (db.prepare("SELECT count(*) n FROM steps WHERE run=? AND state='running'").get(parentRunId).n)
    fail('FACTION_ZERG_GUIDED_REVISION_PARENT_RUNNING');
} finally { db.close(); }
...
if (parentRecipe.version !== 'faction_consumer_evaluation_run_v4'
  || parentCandidate.factionRecordKey !== 'tactical_cards:zerg_swarm')
  fail('FACTION_ZERG_GUIDED_REVISION_PARENT_INVALID');
...
if (finalGeneral.hash !== savedGeneral.hash) fail('FACTION_ZERG_GUIDED_REVISION_GENERAL_DRIFT');
...
const revision = createFactionConsumerGuidedRevisionV3({ input, candidate: parentCandidate,
...
const selectedExecution = selectFactionConsumerExecutionV1();
const profile = selectedExecution.profile;
const limits = { maxCalls: 30, maxCostMicros: 15_000_000, maxTokens: 30_000_000,
  maxWallMs: 90 * 60 * 1000, maxInputBytes: 1_000_000 };
const budgetEpoch = verifySeal(JSON.parse(await readFile(path.join(base,
  'budget-epoch-2026-09-09-reset-v1.json'), 'utf8')));
const recipe = seal({ version: 'faction_consumer_evaluation_run_v5',
  sourceRunId: parentRecipe.sourceRunId, parentConsumerRunId: parentRunId,
  inputHash: input.hash, parentCandidateHash: parentCandidate.hash,
  candidateHash: revision.candidate.hash,
  ...
  contextHash: input.frozenSources.hash, finalGeneralDependencyHash: finalGeneral.hash,
  drillManifestHash: drills.manifest.hash,
  ...
  modelHash: profile.integrity.hash, budgetEpochHash: budgetEpoch.hash, limits,
  ...
  scope: 'bounded_roster_and_grouped_rule_application_after_source_calibrated_zerg_revision',
  runtimeAccepted: false, trainingTruth: false });
const releaseFiles = [
  'scripts/run-ticket-18-zerg-consumer-guided-revision-v3.mjs',
  'packages/skill-production-v3/faction-consumer-guided-revision-v3.mjs',
  'packages/skill-evaluation/faction-consumer-evidence-v1.mjs',
...
const consumerReleaseEvidence = seal({ version: 'faction_consumer_release_evidence_v2',
  criticalRecipeHash: recipe.hash,
  codeHashes: await Promise.all(releaseFiles.map(async file => ({ file,
    hash: sha256(await readFile(path.join(root, file))) }))),
  startupBlocking: false, semanticIdentityAffecting: false,
...
if (args[0] === '--preflight') {
  console.log(JSON.stringify({ ready: true, ticket: 18, slice: 174,
    parentRunId, recipeHash: recipe.hash, parentCandidateHash: parentCandidate.hash,
    revisedCandidateHash: revision.candidate.hash,
    revisionOperations: revision.revisionSpec.operations.map(row => row.id),
    plannedCalls: 20, rosterCalls: 2, groupedRuleCalls: 18,
    actualProviderCalls: 0, callWindowResetsOnLiveStart: true,
    operationalLimits: 'soft_alerts', paymentRequiredStillBlocks: true }));
  process.exit(0);
}

const runId = 'faction-consumer-' + recipe.hash.slice(0, 20);
const out = path.join(base, runId);
...
const store = openProductionStore(filename, { runId, recipeHash: recipe.hash, ...limits,
  resetCallWindow: true, limitMode: 'alert', onLimitAlert: alert =>
    console.log(JSON.stringify({ event: 'consumer-soft-limit-alert', ticket: 18, slice: 174,
      runId, ...alert })) });
...
const historyTokens = 2_864_424;
const historyMicros = 5_052_393 + 28_961_350;
...
function emitAlert(kind, observed, configuredThreshold) {
  if (alerts.has(kind)) return;
  alerts.add(kind);
  console.log(JSON.stringify({ event: 'consumer-soft-limit-alert', ticket: 18, slice: 174,
    runId, kind, observed, configuredThreshold, action: 'continue',
    paymentRequiredStillBlocks: true, trainingTruth: false }));
}
function observeWall() {
  const elapsed = Date.now() - began;
  if (elapsed >= limits.maxWallMs) emitAlert('wall_time', elapsed, limits.maxWallMs);
}
...
try {
  ...
  const epoch = factionBudgetEpochProgressV1({ extension: budgetEpoch, global });
  if (epoch.estimateOrReservedMicros + Math.max(0,
    limits.maxCostMicros - store.summary().reservedOrSettledMicros) > budgetEpoch.allowance.costMicros)
    emitAlert('budget_epoch_cost', epoch.estimateOrReservedMicros, budgetEpoch.allowance.costMicros);
  observeWall();

  await put('recipe', recipe);
  await put('consumer-release-evidence-v2', consumerReleaseEvidence);
  await put('input', input); await put('candidate', revision.candidate);
  await put('parent-candidate', parentCandidate);
  await put('production-evidence', revision.revisionEvidence);
  ...
  await put('source-proof', revision.sourceProof);
  await put('final-general-dependency', finalGeneral);
  await put('drill-manifest', drills.manifest);
  ...
  try {
    attached = await worker.attachCredential({ attachmentId: 'faction-consumer-zerg-' + randomUUID(),
  ...
  const model = createAccountedModel({ store, commandPolicy: 'finish_only',
    ...
    complete: (providerRequest, { signal } = {}) => {
      execution.beforeNewSend?.(); observeWall();
  ...
  rosterResult = await evaluateFactionRosterUseV1({ input, candidate: revision.candidate,
    ...
    onProgress: row => console.log(JSON.stringify({ event: 'consumer-v5', ticket: 18, slice: 174, ...row })) });
  ...
  ruleResult = await evaluateFactionRuleUseGroupedV2({ input, candidate: revision.candidate,
    ...
    onProgress: row => console.log(JSON.stringify({ event: 'rule-consumer-v5', ticket: 18, slice: 174, ...row })) });
  await put('rule-application-evaluation', ruleResult);
  if (!rosterResult.boundedRosterChoicePassed) fail('FACTION_CONSUMER_BOUNDED_EVALUATION_FAILED');
  if (!ruleResult.boundedRuleApplicationPassed) fail('FACTION_RULE_CONSUMER_BOUNDED_EVALUATION_FAILED');
} catch (error) {
  failure = { code: /^[A-Z0-9_]{3,100}$/u.test(error.code || '')
    ? error.code : 'FACTION_ZERG_GUIDED_REVISION_RUN_FAILURE', diagnosticHash: hash(String(error.message)) };
} finally {
  if (attached?.workerRef) await worker.detachCredential({ workerRef: attached.workerRef,
    reason: 'faction_consumer_zerg_revision_finished' }).catch(() => {});
  await worker?.close().catch(() => {});
  const ledger = store.summary();
  const global = store.globalSummary();
  const report = seal({ runId, recipeHash: recipe.hash, resultHash: rosterResult?.hash || null,
    applicationResultHash: ruleResult?.hash || null,
    productionEvidenceHash: revision.revisionEvidence.hash,
    ...
    results: rosterResult?.results.map(row => ({ arm: row.arm,
      correct: row.correct, total: row.total })) || [],
    ruleApplicationSummary: ruleResult?.summary || [],
    groupedRuleCalls: ruleResult?.calls || 0,
  ...
  console.log(JSON.stringify({ event: 'report', runId,
    ...
    rosterResults: report.results, ruleApplicationSummary: report.ruleApplicationSummary,
    failure, cumulativeTokens: report.cumulativeKnownTokensLowerBound,
    cumulativeCny: report.cumulativeEstimateOrReserveCny,
    budgetEpoch: report.budgetEpoch, hash: report.hash }));
  store.close();
}
