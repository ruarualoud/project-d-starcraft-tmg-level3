...
import { repairKnownFactionUnitRoleFieldsV1 } from '../packages/skill-production-v3/faction-unit-role-field-repair-v1.mjs';
...
const transport = createStarcraftTmgProviderEgressTransportV1({ captureResponseOutcome: true,
  ...
  requestImplementation(options, callback) {
    ...
    request.end = body => {
      ...
      assert.equal(hash(w.draft), hash(sectionResult.draft)); assert.equal(w.repairPlan.sourceEvidence.length, 4);
      const output = { replacements: w.editTargetsAtEnd.map(t => ({ targetId: t.targetId,
        text: t.finding.reason })) };
...
const temp = await mkdtemp(path.join(base, 'unit-field-dsh-'));
const store = openProductionStore(path.join(temp, 'fixture.sqlite'), { runId: 'unit-field-dsh', recipeHash: hash(input.hash), maxCalls: 2 });
let result;
try {
  ...
  result = await repairKnownFactionUnitRoleFieldsV1({ input, section: sectionResult.section, draft: sectionResult.draft, store, runtime });
  assert.equal(result.patch.changes.length, 3); assert.equal(result.sourceReviewPassed, false);
  assert.equal((await repairKnownFactionUnitRoleFieldsV1({ input, section: sectionResult.section, draft: sectionResult.draft, store, runtime })).hash, result.hash);
  assert.equal(sends, 1); assert(wireBytes <= transport.metadata().maxRequestBytes);
} finally { store.close(); }
const files = ['packages/skill-production-v3/faction-unit-role-field-repair-v1.mjs', 'packages/skill-production-v3/runtime.mjs',
  'packages/skill-production/loops.mjs', 'packages/skill-production/model.mjs', 'scripts/verify-ticket-18-faction-unit-role-field-dsh-v1.mjs'];
...
await writeFile(path.join(base, 'unit-role-field-dsh-readiness.json'), JSON.stringify(report, null, 2));
