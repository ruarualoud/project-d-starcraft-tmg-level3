...
const EXPECTED_BASE_REPORTS = 123;
const EXPECTED_BASE_ASSERTIONS = 1285;
const EXPECTED_CURRENT = Object.freeze({
  ...
  stateContractMissingExecutors: 17,
});
...
const EXPECTED_EXECUTORS = Object.freeze([
  "authority.academy-medic-ability-v1@1.0.0",
  ...
  "authority.combat-tag-shielded-ranged-v1@1.0.0",
  ...
  "authority.goliath-scatter-ranged-batch-v1@1.0.0",
  ...
  "authority.stimpack-ranged-consumer-v1@1.0.0",
...
await check("generic_runtime_gate_targets_the_current_catalogue_and_executor_manifest", () => {
  ...
  assert.equal(runtimeGate.runtimeDescriptor.nonExecutableRuleAtomCount, 605);
...
await check("relationship_graph_declared_scope_is_closed_without_claiming_global_completion", () => {
  ...
  assert.equal(relationshipAudit.counts.nodes, 6122);
  assert.equal(relationshipAudit.counts.edges, 22043);
  assert.equal(
...
const report = {
  ...
  ctx2skill: {
    ...
    promotions: [],
    blocks: ["remaining_491_actionable_rule_atoms_not_executable"],
