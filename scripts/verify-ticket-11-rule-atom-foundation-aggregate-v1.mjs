...
const EXPECTED_HISTORICAL_UNIT_COMPOSITION_UPGRADE_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_ROSTER_DISCLOSURE_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_MISSION_DEPLOYMENT_DRAFT_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_FACTION_ARMY_ELIGIBILITY_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_RESPAWN_MORPH_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_SUMMON_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_HIDDEN_BURROWED_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_STATUS_STAY_IN_PLAY_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_UNIT_DESTRUCTION_LIFECYCLE_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_RESERVE_LIFECYCLE_RULES = Object.freeze({
...
const EXPECTED_HISTORICAL_SUPPLY_POOL_RULES = Object.freeze({
...
const EXPECTED_EXECUTORS = Object.freeze([
  ...
  "authority.army-resource-budget-rules-v1@1.0.0",
  ...
  "authority.deployment-geometry-rules-v1@1.0.0",
  ...
  "authority.faction-army-eligibility-rules-v1@1.0.0",
  ...
  "authority.hidden-burrowed-rules-v1@1.0.0",
  ...
  "authority.mission-deployment-draft-rules-v1@1.0.0",
  ...
  "authority.reserve-lifecycle-rules-v1@1.0.0",
  ...
  "authority.status-stay-in-play-rules-v1@1.0.0",
  ...
  "authority.summon-rules-v1@1.0.0",
  ...
  "authority.unit-composition-upgrade-rules-v1@1.0.0",
  "authority.unit-destruction-lifecycle-rules-v1@1.0.0",
...
await check("current_slice_catalogue_runtime_and_lineage_are_exact", () => {
  ...
  assert.equal(historicalMissionDeploymentDraftRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_ROSTER_DISCLOSURE_RULES.sliceHash,
  );
  assert.equal(historicalMissionDeploymentDraftRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_ROSTER_DISCLOSURE_RULES.catalogueHash,
  );
  assert.equal(historicalRosterDisclosureRules.runtimeHash,
    EXPECTED_HISTORICAL_ROSTER_DISCLOSURE_RULES.runtimeHash);
  assert.equal(historicalRosterDisclosureRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_UNIT_COMPOSITION_UPGRADE_RULES.sliceHash);
  assert.equal(historicalRosterDisclosureRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_UNIT_COMPOSITION_UPGRADE_RULES.catalogueHash);
  assert.equal(historicalUnitCompositionUpgradeRules.runtimeHash,
    EXPECTED_HISTORICAL_UNIT_COMPOSITION_UPGRADE_RULES.runtimeHash);
  assert.equal(historicalUnitCompositionUpgradeRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_ARMY_RESOURCE_BUDGET_RULES.sliceHash);
  assert.equal(historicalUnitCompositionUpgradeRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_ARMY_RESOURCE_BUDGET_RULES.catalogueHash);
  assert.equal(historicalArmyResourceBudgetRules.runtimeHash,
    EXPECTED_HISTORICAL_ARMY_RESOURCE_BUDGET_RULES.runtimeHash);
  assert.equal(historicalArmyResourceBudgetRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_FACTION_ARMY_ELIGIBILITY_RULES.sliceHash);
  assert.equal(historicalArmyResourceBudgetRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_FACTION_ARMY_ELIGIBILITY_RULES.catalogueHash);
  assert.equal(historicalFactionArmyEligibilityRules.runtimeHash,
    EXPECTED_HISTORICAL_FACTION_ARMY_ELIGIBILITY_RULES.runtimeHash);
  assert.equal(historicalFactionArmyEligibilityRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_RESPAWN_MORPH_RULES.sliceHash);
  assert.equal(historicalFactionArmyEligibilityRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_RESPAWN_MORPH_RULES.catalogueHash);
  assert.equal(historicalRespawnMorphRules.runtimeHash,
    EXPECTED_HISTORICAL_RESPAWN_MORPH_RULES.runtimeHash);
  assert.equal(historicalRespawnMorphRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_SUMMON_RULES.sliceHash);
  assert.equal(historicalRespawnMorphRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_SUMMON_RULES.catalogueHash);
  assert.equal(historicalSummonRules.runtimeHash,
    EXPECTED_HISTORICAL_SUMMON_RULES.runtimeHash);
  assert.equal(historicalSummonRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_HIDDEN_BURROWED_RULES.sliceHash);
  assert.equal(historicalSummonRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_HIDDEN_BURROWED_RULES.catalogueHash);
  assert.equal(historicalHiddenBurrowedRules.runtimeHash,
    EXPECTED_HISTORICAL_HIDDEN_BURROWED_RULES.runtimeHash);
  assert.equal(historicalHiddenBurrowedRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_STATUS_STAY_IN_PLAY_RULES.sliceHash);
  assert.equal(historicalHiddenBurrowedRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_STATUS_STAY_IN_PLAY_RULES.catalogueHash);
  assert.equal(historicalStatusStayInPlayRules.runtimeHash,
    EXPECTED_HISTORICAL_STATUS_STAY_IN_PLAY_RULES.runtimeHash);
  assert.equal(historicalStatusStayInPlayRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_UNIT_DESTRUCTION_LIFECYCLE_RULES.sliceHash);
  assert.equal(historicalStatusStayInPlayRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_UNIT_DESTRUCTION_LIFECYCLE_RULES.catalogueHash);
  assert.equal(historicalUnitDestructionLifecycleRules.runtimeHash,
    EXPECTED_HISTORICAL_UNIT_DESTRUCTION_LIFECYCLE_RULES.runtimeHash);
  assert.equal(historicalUnitDestructionLifecycleRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_RESERVE_LIFECYCLE_RULES.sliceHash);
  assert.equal(historicalUnitDestructionLifecycleRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_RESERVE_LIFECYCLE_RULES.catalogueHash);
  assert.equal(historicalReserveLifecycleRules.runtimeHash,
    EXPECTED_HISTORICAL_RESERVE_LIFECYCLE_RULES.runtimeHash);
  assert.equal(historicalReserveLifecycleRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_SUPPLY_POOL_RULES.sliceHash);
  assert.equal(historicalReserveLifecycleRules.slice.previousCatalogueHash,
    EXPECTED_HISTORICAL_SUPPLY_POOL_RULES.catalogueHash);
  assert.equal(historicalSupplyPoolRules.runtimeHash,
    EXPECTED_HISTORICAL_SUPPLY_POOL_RULES.runtimeHash);
  assert.equal(historicalSupplyPoolRules.slice.previousSliceHash,
    EXPECTED_HISTORICAL_ROUND_PHASE_ACTIVATION_RULES.sliceHash);
  assert.equal(historicalSupplyPoolRules.slice.previousCatalogueHash,
...
await check("historical_v4_catalogue_runtime_and_rules_display_remain_frozen", () => {
  ...
  assert.equal(
    historicalMissionDeploymentDraftRules.slice.historicalCompatibility.previousCatalogueHash,
    EXPECTED_HISTORICAL_ROSTER_DISCLOSURE_RULES.catalogueHash,
  );
  assert.equal(
    historicalMissionDeploymentDraftRules.slice.historicalCompatibility.previousRuntimeHash,
    EXPECTED_HISTORICAL_ROSTER_DISCLOSURE_RULES.runtimeHash,
  );
  assert.equal(
    historicalReserveLifecycleRules.slice.historicalCompatibility.previousSliceHash,
    EXPECTED_HISTORICAL_SUPPLY_POOL_RULES.sliceHash,
  );
  assert.equal(
    historicalReserveLifecycleRules.slice.historicalCompatibility.previousCatalogueHash,
    EXPECTED_HISTORICAL_SUPPLY_POOL_RULES.catalogueHash,
  );
  assert.equal(
    historicalReserveLifecycleRules.slice.historicalCompatibility.previousRuntimeHash,
    EXPECTED_HISTORICAL_SUPPLY_POOL_RULES.runtimeHash,
  );
  assert.equal(
...
await check("generic_runtime_gate_targets_the_current_catalogue_and_executor_manifest", () => {
  ...
  assert.equal(
    runtimeGate.runtimeDescriptor.nonExecutableRuleAtomCount,
    EXPECTED_CURRENT.reviewRequiredRuleAtoms + EXPECTED_CURRENT.displayOnlyRuleAtoms,
  );
