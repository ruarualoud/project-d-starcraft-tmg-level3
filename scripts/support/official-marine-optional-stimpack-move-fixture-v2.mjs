...
function supplyFor(currentModels) {
  ...
  throw new Error("OPTIONAL_STIMPACK_FIXTURE_MODEL_COUNT_UNSUPPORTED");
}

export async function createOfficialMarineOptionalStimpackMoveFixtureV2(input = {}) {
  ...
  function marine(pieceInput) {
    ...
    const currentModels = Number(pieceInput.currentModels || 1);
    const maxModels = Number(pieceInput.maxModels || currentModels);
    const positions = Array.from({ length: currentModels }, (_, index) => ({
      xInches: pieceInput.xInches + ((index % 3) * 1.5),
      yInches: pieceInput.yInches + (Math.floor(index / 3) * 1.5),
    }));
    return {
      id: pieceInput.id,
      ...
      destroyedModelIds: Array.from(
        ...
        (_, index) => `${pieceInput.id}-destroyed-${currentModels + index + 1}`,
      ),
      isOnField: true,
      isInReserves: false,
      isDestroyed: false,
      ...
      models: positions.map((position, index) => model(
  ...
  function battleState(options = {}) {
    const stimpackSide = options.stimpackSide || "player1";
    const normalSide = stimpackSide === "player1" ? "player2" : "player1";
    const currentModels = Number(options.currentModels || 1);
    const maxModels = Number(options.maxModels || currentModels);
    const opponentModels = Number(options.opponentModels || 1);
    const paymentReadiness = options.paymentReadiness || "ready";
    ...
    const state = {
      ...
      cardResources: {
        player1: [card("p1-taf", "player1", stimpackSide === "player1"
          ? paymentReadiness
          : "ready")],
        player2: [card("p2-taf", "player2", stimpackSide === "player2"
          ? paymentReadiness
          : "ready")],
      },
      pieces: [
        ...
        marine({
          ...
          xInches: 25,
          yInches: 5,
          currentModels: opponentModels,
          maxModels: opponentModels,
        }),
      ],
      gameOver: false,
      terminal: false,
