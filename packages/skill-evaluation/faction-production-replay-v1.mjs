...
export function openFactionProductionReplayV1({ filename, runId, recipe, ancestors = [] }) {
  ...
  function verifyRole(value, id, expectedContextHash, expectedInputHash) {
    ...
    value.loop.transcript.forEach((t, index) => {
      ...
      if (hash(body) !== receiptHash || body.schemaVersion !== 'starcraft_tmg_provider_egress_transport_v1.success'
        ...
        || t.call !== index + 1 || t.action !== command.action || t.commandHash !== hash(command)) fail('FACTION_REPLAY_RECEIPT_INVALID');
  ...
  const store = Object.freeze({
    acquire(id, input) {
      ...
      if (row.value?.roleId === id) {
        verifyRole(row.value, id, input.contextHash, row.inputHash);
