...
import { assertStarcraftTmgProviderWorkerSuccessV1 } from
  "./provider-worker-success-classifier-v1.mjs";

// Wire-compatible with the frozen v1 parent port. The executable identity is
// independently exposed by provider-egress-worker-port-v2.mjs.
...
async function initialize(message) {
  ...
  if (initialized || closing) {
    throw new TypeError("Provider Worker cannot initialize twice");
  }
...
async function complete(message) {
  ...
  try {
    ...
    assertStarcraftTmgProviderWorkerSuccessV1(value, {
      providerRequestId: message.providerRequest.requestId,
      egressBinding,
