...
export function inspectOverallExamsV3({ candidate, exam, regression, drills, legacyDrills, probes,
  readStep, readReceipt, providerProfileHash, maxWallMs = 300000 }) {
  ...
  if (!exam.passed || !regression.passed) fail('OVERALL_EVIDENCE_QUALITY_NOT_PASSED');
...
export function inspectCompletedOverallProductionV3({ filename, recipe, report, candidate, exam, regression,
  plan, catalogue, context, drills, legacyDrills, probes }) {
  ...
  if (recipe.version !== 'overall-rules-production-v3-complete-and-exam' || report.runId !== runId
    || report.recipeHash !== recipe.hash || report.failure || recipe.planHash !== plan.hash
  ...
  try {
    ...
    const exams = inspectOverallExamsV3({ candidate, exam, regression, drills, legacyDrills, probes, readStep,
      providerProfileHash: recipe.modelHash, readReceipt: h => responses.find(r => r.usageReceipt?.receiptHash === h) });
    if (report.actualExamHash !== exam.hash || report.sourceRegressionHash !== regression.hash
      || hash(report.actualExamSummary) !== hash(exam.summary) || !report.actualExamPassed
      || !report.sourceRegressionPassed || !report.overallEvaluationPassed) fail('OVERALL_EVIDENCE_REPORT_DRIFT');
