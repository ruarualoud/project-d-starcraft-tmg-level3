...
export function createFactionUnitRoleFieldPlanV1({ input, section, draft, draftEnvelopeBinding = null }) {
...
export function applyFactionUnitRoleFieldRepairV1(output, { input, section, draft, plan, draftEnvelopeBinding = null }) {
  verifySeal(plan);
  if (createFactionUnitRoleFieldPlanV1({ input, section, draft, draftEnvelopeBinding }).hash !== plan.hash) fail('FACTION_UNIT_FIELD_PLAN_DRIFT');
...
export async function repairKnownFactionUnitRoleFieldsV1({ input, section, draft, runtime, store, draftEnvelopeBinding = null }) {
  const plan = createFactionUnitRoleFieldPlanV1({ input, section, draft, draftEnvelopeBinding });
  ...
  const patch = applyFactionUnitRoleFieldRepairV1(edited.output, { input, section, draft, plan, draftEnvelopeBinding });
