export const STARCRAFT_TMG_CLIENT_CANONICALIZATION = "RFC8785";
export const STARCRAFT_TMG_CLIENT_HASH_ALGORITHM = "sha256";
...
function normalizeCanonical(value, path = "$") {
  ...
  if (Array.isArray(value)) {
    return value.map((entry, index) => {
      if (entry === undefined || ["function", "symbol", "bigint"].includes(typeof entry)) {
...
export function canonicalStarcraftTmgClientJson(value) {
...
export function hashStarcraftTmgClientContract(value) {
  return sha256(utf8Bytes(canonicalStarcraftTmgClientJson(value)));
}
