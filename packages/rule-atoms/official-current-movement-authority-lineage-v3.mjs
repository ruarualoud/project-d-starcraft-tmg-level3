...
import {
  OFFICIAL_SELECTED_UPGRADE_LOADOUT_BINDING_V2_SCHEMA,
  ...
  OFFICIAL_START_OF_ROUND_V5_HISTORY_SCHEMA,
  OFFICIAL_START_OF_ROUND_V5_RESOLUTION_SCHEMA,
...
export const OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V3_SCHEMA =
  "starcraft_tmg_current_movement_authority_lineage_v3";
export const OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V3_CONTRACT =
  "start-of-round-v5-marine-medic-loadout+phase-initiative-v1+explicit-supply-mutation-lineage-v3";
...
const SUPPLY_MUTATION_EXECUTORS = Object.freeze(new Map([
  ["reserve_deployed", new Set(["authority.reserve-deploy-v5@5.0.0"])],
  ["unit_disengaged", new Set(["authority.disengage-v5@5.0.0"])],
  ["unit_disengage_failed", new Set(["authority.disengage-v5@5.0.0"])],
]));
...
function verifyLoadouts(state, resolution, history, event, prefix) {
  const rows = resolution.selectedUpgradeLoadouts;
  if (!Array.isArray(rows)
    || !HASH_PATTERN.test(String(resolution.selectedUpgradeLoadoutHash || ""))
    || resolution.selectedUpgradeLoadoutHash !== history.selectedUpgradeLoadoutHash
    || resolution.selectedUpgradeLoadoutHash !== event.selectedUpgradeLoadoutHash
    || hashStarcraftTmgContract({
      schema: OFFICIAL_SELECTED_UPGRADE_LOADOUT_BINDING_V2_SCHEMA,
      rows,
    }) !== resolution.selectedUpgradeLoadoutHash) {
    fail(prefix, "START_OF_ROUND_HANDOFF_INVALID", "selected_upgrade_binding");
  }
  const stateRows = (state.pieces || []).map((piece) => ({
  ...
  if (!isDeepStrictEqual(rows, stateRows)) {
    fail(prefix, "START_OF_ROUND_HANDOFF_INVALID", "selected_upgrade_state_drift");
  }
}
...
function verifyStartOfRound(state, prefix) {
  ...
  if (!exactKeys(history, [
    ...
    "selectedUpgradeLoadoutHash",
    "stationaryPieceIds",
    "readyCardIds",
    "nextPhase",
    "trainingTruth",
  ])
    || history.schema !== OFFICIAL_START_OF_ROUND_V5_HISTORY_SCHEMA
    ...
    || !HASH_PATTERN.test(String(history.selectedUpgradeLoadoutHash || ""))
  ...
  const indexes = logs.flatMap((entry, index) => (
    Number(entry?.round) === Number(state.round)
      ...
      && entry?.action?.executorId === OFFICIAL_START_OF_ROUND_V5_EXECUTOR_ID
      && entry?.action?.executorVersion === OFFICIAL_START_OF_ROUND_V5_EXECUTOR_VERSION
      ? [index]
      : []
  ));
  if (indexes.length !== 1) {
    fail(prefix, "START_OF_ROUND_HANDOFF_INVALID", "current_v5_log");
  }
  ...
  if (!exactKeys(action, [
    ...
    || action.actionType !== OFFICIAL_START_OF_ROUND_V5_ACTION_TYPE
    ...
    || !isDeepStrictEqual(action.ruleAtomIds, [...OFFICIAL_START_OF_ROUND_V5_EXECUTOR_ATOM_IDS])
    || !hashBound(resolution, "startOfRoundResolutionHash")
    || resolution.schema !== OFFICIAL_START_OF_ROUND_V5_RESOLUTION_SCHEMA
    ...
    fail(prefix, "START_OF_ROUND_HANDOFF_INVALID", "current_v5_material");
  }
  verifyLoadouts(state, resolution, history, event, prefix);
  try {
...
export function verifyOfficialCurrentMovementAuthorityLineageV3(state, options = {}) {
  const errorPrefix = String(options.errorPrefix || "CURRENT_MOVEMENT_V3").trim();
  if (!/^[A-Z][A-Z0-9_]*$/u.test(errorPrefix)) {
    throw new Error("CURRENT_MOVEMENT_V3_ERROR_PREFIX_INVALID");
  }
  ...
  const body = {
    schema: OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V3_SCHEMA,
    contract: OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V3_CONTRACT,
    round: Number(state.round),
    phase: "movement",
    ...
    selectedUpgradeLoadoutHash: start.history.selectedUpgradeLoadoutHash,
