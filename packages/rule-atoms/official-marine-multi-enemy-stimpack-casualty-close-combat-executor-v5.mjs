...
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CLOSE_COMBAT_EXECUTOR_ID =
  "authority.marine-multi-enemy-stimpack-casualty-close-combat-v5";
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CLOSE_COMBAT_EXECUTOR_VERSION =
  "5.0.0";
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CLOSE_COMBAT_TRANSITION_SCHEMA =
  "starcraft_tmg_authority_v3.receipt";
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_ATTACK_ACTION_TYPE =
  "fight";
export const OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_STIMPACK_PRECISION_ACTION_TYPE =
  "resolve_multi_enemy_stimpack_close_combat_precision";
export const OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_STIMPACK_CASUALTIES_ACTION_TYPE =
  "resolve_multi_enemy_stimpack_close_combat_casualties";
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_PRECISION_PENDING_SCHEMA =
  "starcraft_tmg_official_marine_multi_enemy_stimpack_precision_pending_v3";
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PENDING_SCHEMA =
  "starcraft_tmg_official_marine_multi_enemy_stimpack_casualty_pending_v3";
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_NEW_ATOM_IDS =
  Object.freeze([]);
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_ACTION_ATOM_IDS =
  OFFICIAL_MARINE_MULTI_MODEL_CASUALTY_CLOSE_COMBAT_ACTION_ATOM_IDS;
export const OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_EXECUTOR_ATOM_IDS =
...
function otherSide(sideKey) {
  ...
  fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_SIDE_REQUIRED");
}

function runtimeHash(options) {
  const value = String(options.matchBinding?.rulesRuntimeBinding?.runtimeHash || "");
  if (!/^[a-f0-9]{64}$/u.test(value)) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_RUNTIME_HASH_REQUIRED");
  }
  return value;
}
...
function statusFor(piece, plan) {
  ...
  if (rows.length !== 1) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PRECISION_STATUS_STALE");
  }
  return rows[0];
}
...
function contextFor(state, sideKey, attackerPieceId, targetPieceId, matchBinding) {
  ...
  const coEngager = state.pieces.find((entry) => entry.id === plan.coEngagerPieceId);
  const target = state.pieces.find((entry) => entry.id === targetPieceId);
  const status = statusFor(piece, plan);
  const grant = PRECISION.createGrant({ plan: plan.selectedPairPlan, status });
  return { plan, piece, coEngager, target, status, grant };
}

function attackAction(context) {
  const plan = context.plan;
  return {
    actionType: OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_ATTACK_ACTION_TYPE,
    sideKey: plan.sideKey,
    phase: "combat",
    pieceId: plan.attackerPieceId,
    coEngagerPieceId: plan.coEngagerPieceId,
    ...
    resolutionMode: "multi_enemy_stimpack_precision_then_casualty_choice",
    ...
    ruleAtomIds: [...OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_ACTION_ATOM_IDS],
    executorId: OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CLOSE_COMBAT_EXECUTOR_ID,
    executorVersion:
...
function precisionAction(pending, selection) {
  return {
    actionType: OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_STIMPACK_PRECISION_ACTION_TYPE,
    sideKey: pending.ownerSideKey,
    phase: "combat",
    pieceId: pending.attackerPieceId,
    coEngagerPieceId: pending.coEngagerPieceId,
...
function casualtyAction(pending, selection) {
  return {
    actionType: OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_STIMPACK_CASUALTIES_ACTION_TYPE,
    sideKey: pending.ownerSideKey,
    phase: "combat",
    pieceId: pending.attackerPieceId,
    coEngagerPieceId: pending.coEngagerPieceId,
...
function verifyPending(pending, schema) {
  if (!object(pending)
    ...
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PENDING_INVALID");
  }
  return pending;
}
...
function currentContext(state, pending, options) {
  ...
  if (!isDeepStrictEqual(context.plan, pending.attackPlan)
    || context.plan.coEngagerPieceId !== pending.coEngagerPieceId) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PENDING_STATE_DRIFT");
  }
  return context;
}
...
function verifyPrecisionPending(state, options) {
  ...
  if (!object(pending.precisionGrant)
    ...
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PRECISION_PENDING_INVALID");
  }
  const context = currentContext(state, pending, options);
  if (!isDeepStrictEqual(context.grant, pending.precisionGrant)) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PRECISION_PENDING_STATE_DRIFT");
  }
  PRECISION.verifyGrant({
    plan: context.plan.selectedPairPlan,
...
function createCasualtyDomain(state, context, attackResolution, options) {
...
function verifyCasualtyPending(state, options) {
  ...
  if (!object(pending.attackResolution)
    ...
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CHOICE_PENDING_INVALID");
  }
  ...
  if (!isDeepStrictEqual(domain, pending.casualtyDomain)) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CHOICE_PENDING_STATE_DRIFT");
  }
  return { pending, context, domain };
}

export function isOfficialMarineMultiEnemyStimpackCasualtyPendingV5(state) {
  return [
    OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_PRECISION_PENDING_SCHEMA,
    OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PENDING_SCHEMA,
...
export function enumerateOfficialMarineMultiEnemyStimpackCasualtyCloseCombatV5(
  state,
  options = {},
) {
  ...
  try {
    if (state?.pendingAction?.schema
      ...
      if (state.phase !== "combat"
        ...
        || sideKey !== pending.ownerSideKey) {
        fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PENDING_WRONG_SEAT");
      }
      return pending.precisionSelections.map((selection) => ({
        ...precisionAction(pending, selection),
        isEnabled: true,
        disabledReason: "",
        score: 460 + selection.convertedCount,
        details: {
          ...
          coEngagerPieceId: pending.coEngagerPieceId,
          rulesTruth: "official_multi_enemy_stimpack_precision_choice_domain_v5",
          trainingTruth: false,
        },
      }));
    }
        ...
        try {
          ...
          rows.push({
            ...
            details: {
              ...
              coEngagerModelLedgerHash: context.plan.coEngagerLedger.modelLedgerHash,
              targetModelLedgerHash: context.plan.targetLedger.modelLedgerHash,
              targetEngagedEnemyUnitIds: [...context.plan.targetEngagedEnemyUnitIds],
              ...
              rulesTruth:
                "official_current_multi_enemy_stimpack_precision_casualty_consumer",
              trainingTruth: false,
            },
          });
        } catch (error) {
...
function transition(state, action, events, options, extras = {}) {
  return {
    ...
    events,
    action,
    rulesTruth: extras.rulesTruth || "official_multi_enemy_stimpack_casualty_v5",
    trainingTruth: false,
    ...extras,
  };
}
...
function openPrecision(stateInput, expected, context, options) {
  ...
  const body = {
    schema: OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_PRECISION_PENDING_SCHEMA,
    ...
    coEngagerPieceId: context.plan.coEngagerPieceId,
  ...
  const events = [{
    type: "marine_multi_enemy_stimpack_precision_choice_opened",
    sideKey: pending.ownerSideKey,
    pieceId: pending.attackerPieceId,
    coEngagerPieceId: pending.coEngagerPieceId,
    targetId: pending.targetPieceId,
  ...
  return transition(state, expected, events, options, {
    ...
    rulesTruth: "official_multi_enemy_stimpack_precision_choice_opened_v5",
  });
}
...
function openCasualtyChoice(
  ...
  const body = {
    schema: OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PENDING_SCHEMA,
    round: Number(stateInput.round),
    phase: "combat",
  ...
  state.activeSideKey = pending.ownerSideKey;
  const events = [{
    type: "marine_multi_enemy_stimpack_casualty_choice_opened",
    sideKey: pending.ownerSideKey,
  ...
  return transition(state, expected, events, options, {
    ...
    attackResolution,
    casualtyDomain,
    rulesTruth: "official_defender_owned_multi_enemy_stimpack_casualty_choice_opened",
  });
}
...
function finishCasualties(
  ...
  const event = {
    ...
    subtype: "marine_multi_enemy_stimpack_precision_casualty_consumer",
    ...
    attackResolutionHash: attackResolution.attackResolutionHash,
    precisionResolutionHash: attackResolution.precisionResolutionHash,
    casualtyKernelHash: CASUALTIES.descriptor.kernelHash,
    casualtyDomainHash: casualtyDomain.domainHash,
    casualtySelectionHash: selection.selectionHash,
  ...
  return transition(state, expected, events, options, {
    ...
    rulesTruth: "official_current_multi_enemy_stimpack_casualty_settlement",
  });
}

function stageCasualties(stateInput, expected, context, attackResolution, options) {
  const projected = projectedAttackState(stateInput, context.plan.sideKey);
  const domain = createCasualtyDomain(projected, context, attackResolution, options);
...
function resolvePrecision(stateInput, action, expected, options) {
  ...
  if (!selection || !isDeepStrictEqual(
    ...
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_PRECISION_SELECTION_STALE");
  }
  const resolution = PRECISION.resolve({
    plan: pending.attackPlan.selectedPairPlan,
    grant: pending.precisionGrant,
    status: context.status,
  ...
  const attackResolution = {
    schema: "starcraft_tmg_official_marine_multi_enemy_stimpack_attack_resolution_v5",
    attackPlanHash: context.plan.planHash,
...
function resolveCasualtyChoice(stateInput, action, expected, options) {
  ...
  if (!selection || !isDeepStrictEqual(selection.casualtyModelIds, action.casualtyModelIds)) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_SELECTION_STALE");
  }
  return finishCasualties(
    stateInput,
    expected,
...
export function applyOfficialMarineMultiEnemyStimpackCasualtyCloseCombatV5(
  stateInput,
  actionInput,
  options = {},
) {
  if (!object(actionInput)
    ...
    || actionInput.executorVersion
      !== OFFICIAL_MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_CLOSE_COMBAT_EXECUTOR_VERSION) {
    fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_ACTION_INVALID");
  }
  const expected = enumerateOfficialMarineMultiEnemyStimpackCasualtyCloseCombatV5(
    stateInput,
    {
  ...
  if (!expected) fail("MARINE_MULTI_ENEMY_STIMPACK_CASUALTY_ACTION_STALE");
  ...
  return openPrecision(stateInput, expected, context, options);
}
