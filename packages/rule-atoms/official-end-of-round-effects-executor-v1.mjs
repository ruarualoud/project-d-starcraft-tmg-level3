...
export const OFFICIAL_END_OF_ROUND_EFFECTS_ACTION_TYPE = "resolve_end_of_round_effects";
export const OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_ID =
  "authority.end-of-round-effects-v1";
export const OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_VERSION = "1.0.0";
export const OFFICIAL_END_OF_ROUND_EFFECTS_TRANSITION_SCHEMA =
  "starcraft_tmg_authority_v2.receipt";

export const OFFICIAL_END_OF_ROUND_EFFECTS_NEW_ATOM_IDS = Object.freeze([
  "rule-atom:end-of-round-effect-window",
  "rule-atom:singleton:core-12-6-end-round-effects-step:c3999786b906",
].sort((left, right) => left.localeCompare(right)));
export const OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_ATOM_IDS =
  OFFICIAL_END_OF_ROUND_EFFECTS_NEW_ATOM_IDS;

const PROGRESS_SCHEMA = "starcraft_tmg_scoring_cleanup_progress_v1";
const PROOF_SCHEMA = "starcraft_tmg_official_end_of_round_effect_queue_proof_v1";
const HISTORY_SCHEMA = "starcraft_tmg_official_end_of_round_effect_history_entry_v1";
...
function supportedOfficialSources(state, gameplayDataBundle) {
  ...
  if (!object(state.cardResources)
    || !SIDE_KEYS.every((sideKey) => Array.isArray(state.cardResources[sideKey])
      && state.cardResources[sideKey].length === 0)
    || !object(state.board)
  ...
  if (sourceMaterial.length === 0 || new Set(sourceMaterial.map((row) => row.pieceId)).size
      !== sourceMaterial.length || sourceMaterial.some((row) => !row.pieceId)) {
    fail("END_OF_ROUND_EFFECTS_SOURCE_SCOPE_UNRESOLVED");
  }
  return sourceMaterial.sort((left, right) => left.pieceId.localeCompare(right.pieceId));
}
...
function effectQueueProof(state, context) {
  ...
  const body = {
    ...
    rulesTruth: "official_current_hold_position_unupgraded_marine_empty_eor_queue_only",
    trainingTruth: false,
  };
  return {
    ...body,
...
function action(context) {
  return {
    ...
    ruleAtomIds: [...OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_ATOM_IDS],
...
function disabledAction(state, sideKey, error) {
  return {
    ...
    score: 0,
    details: {
      rulesTruth: "official_end_of_round_effects_fail_closed",
      trainingTruth: false,
    },
  };
}

export function enumerateOfficialEndOfRoundEffectsActionsV1(state, options = {}) {
...
export function applyOfficialEndOfRoundEffectsV1(stateInput, actionInput, options = {}) {
  if (!object(actionInput)
    || actionInput.actionType !== OFFICIAL_END_OF_ROUND_EFFECTS_ACTION_TYPE
    || actionInput.executorId !== OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_ID
    || actionInput.executorVersion !== OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_VERSION) {
  ...
  return {
    ok: true,
    schemaVersion: "starcraft_tmg_official_end_of_round_effects_transition_v1",
    executorId: OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_ID,
    executorVersion: OFFICIAL_END_OF_ROUND_EFFECTS_EXECUTOR_VERSION,
...
export const OFFICIAL_END_OF_ROUND_EFFECTS_DEPENDENCY_ATOM_IDS =
  OFFICIAL_HOLD_POSITION_END_GAME_ATOM_IDS;
