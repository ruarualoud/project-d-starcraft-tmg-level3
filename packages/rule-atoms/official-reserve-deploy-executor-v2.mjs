...
export const OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_ID = "authority.reserve-deploy-v2";
export const OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_VERSION = "2.0.0";
export const OFFICIAL_RESERVE_DEPLOY_V2_TRANSITION_SCHEMA =
  "starcraft_tmg_authority_v2.receipt";
export const OFFICIAL_RESERVE_DEPLOY_V2_PARAMETER_KIND =
  "official_reserve_deploy_path_v2";
export const OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_ATOM_IDS = Object.freeze([
  ...OFFICIAL_RESERVE_DEPLOY_NEW_ATOM_IDS,
]);
export const OFFICIAL_RESERVE_DEPLOY_V2_ACTION_ATOM_IDS = Object.freeze([
  ...OFFICIAL_RESERVE_DEPLOY_ACTION_ATOM_IDS,
]);
...
function verifyStartOfRoundHandoff(state) {
  ...
  if (!exactKeys(history, [
    ...
    fail("RESERVE_DEPLOY_V2_START_OF_ROUND_HANDOFF_INVALID");
  }
  ...
  if (startIndexes.length !== 1) {
    fail("RESERVE_DEPLOY_V2_START_OF_ROUND_HANDOFF_INVALID", "current_v2_log");
  }
  ...
  if (!exactKeys(action, [
    ...
    fail("RESERVE_DEPLOY_V2_START_OF_ROUND_HANDOFF_INVALID", "current_v2_material");
  }
...
function verifyPhaseHandoff(state, startHandoff) {
  ...
  if (!exactKeys(choice, [
    ...
    fail("RESERVE_DEPLOY_V2_PHASE_HANDOFF_INVALID");
  }
  ...
  if (matches.length !== 1) fail("RESERVE_DEPLOY_V2_PHASE_HANDOFF_INVALID", "current_log");
  const log = matches[0];
  ...
  if (!exactKeys(action, [
    ...
    fail("RESERVE_DEPLOY_V2_PHASE_HANDOFF_INVALID", "current_material");
  }
}
...
function verifyRoundSupplyLineage(state, startHandoff) {
  ...
  if (!hashBound(current, "roundSupplyStateHash")
    ...
    fail("RESERVE_DEPLOY_V2_SUPPLY_LINEAGE_INVALID", "current_state");
  }
  ...
  for (const log of startHandoff.logs.slice(startHandoff.startIndex + 1)) {
    for (const event of Array.isArray(log?.events) ? log.events : []) {
      ...
      if (!allowed
        ...
        || before !== expectedHash) {
        fail("RESERVE_DEPLOY_V2_SUPPLY_LINEAGE_INVALID", String(event?.type || "unknown"));
      }
      expectedHash = after;
    }
  }
  if (expectedHash !== current.roundSupplyStateHash) {
    fail("RESERVE_DEPLOY_V2_SUPPLY_LINEAGE_INVALID", "unwitnessed_change");
  }
}
...
function diagnosticAction(state, sideKey, pieceId, error) {
  return {
    ...
    phase: "movement",
    pieceId,
    ruleAtomIds: [...OFFICIAL_RESERVE_DEPLOY_V2_ACTION_ATOM_IDS],
    ...
    score: 0,
    details: {
      rulesTruth: "official_reserve_deploy_v2_fail_closed",
...
function currentDomain(legacyDomain) {
  const core = {
    ...clone(legacyDomain),
    parameterKind: OFFICIAL_RESERVE_DEPLOY_V2_PARAMETER_KIND,
    executorId: OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_ID,
    executorVersion: OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_VERSION,
    rulesTruth: "official_current_start_v2_phase_v1_reserve_deploy_parameter_domain_v2",
    trainingTruth: false,
  };
...
function currentAction(legacyAction) {
  return {
    ...clone(legacyAction),
    executorId: OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_ID,
    executorVersion: OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_VERSION,
...
export function enumerateOfficialReserveDeployV2(state, options = {}) {
  ...
  const legacy = enumerateOfficialReserveDeployV1(state, options);
  return {
    ...
    parameterDomains: legacy.parameterDomains.map(currentDomain),
  };
}

export function instantiateOfficialReserveDeployV2(state, domain, parameters, options = {}) {
  if (!object(domain)
    || domain.parameterKind !== OFFICIAL_RESERVE_DEPLOY_V2_PARAMETER_KIND
    ...
    fail("RESERVE_DEPLOY_V2_PARAMETER_DOMAIN_INVALID");
  }
  const current = enumerateOfficialReserveDeployV2(state, {
  ...
  if (!expected || !isDeepStrictEqual(domain, expected)) {
    fail("RESERVE_DEPLOY_V2_PARAMETER_DOMAIN_STALE");
  }
  const legacy = enumerateOfficialReserveDeployV1(state, {
    sideKey: domain.sideKey,
    includeDisabled: true,
    matchBinding: options.matchBinding,
  });
  const legacyDomain = legacy.parameterDomains.find((entry) => entry.pieceId === domain.pieceId);
  if (!legacyDomain) fail("RESERVE_DEPLOY_V2_PARAMETER_DOMAIN_STALE");
  ...
  return {
    ...
    rulesTruth: "official_current_start_v2_phase_v1_reserve_deploy_instantiation_v2",
...
export function applyOfficialReserveDeployV2(stateInput, actionInput, options = {}) {
  if (!object(actionInput)
    || actionInput.actionType !== "deploy"
    || actionInput.executorId !== OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_ID
    || actionInput.executorVersion !== OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_VERSION
    || !object(actionInput.deployPlan)) {
    fail("RESERVE_DEPLOY_V2_ACTION_INVALID");
  }
  const enumeration = enumerateOfficialReserveDeployV2(stateInput, {
    sideKey: actionInput.sideKey,
    includeDisabled: true,
    matchBinding: options.matchBinding,
  });
  const domain = enumeration.parameterDomains.find((entry) => entry.pieceId === actionInput.pieceId);
  if (!domain) fail("RESERVE_DEPLOY_V2_PARAMETER_DOMAIN_STALE");
  const instantiated = instantiateOfficialReserveDeployV2(stateInput, domain, {
  ...
  if (!isDeepStrictEqual(actionInput, instantiated.action)) {
    fail("RESERVE_DEPLOY_V2_ACTION_MISMATCH");
  }
  const frozen = applyOfficialReserveDeployV1(
    stateInput,
    legacyAction(actionInput),
    options,
  );
  ...
  return {
    ...frozen,
    schemaVersion: "starcraft_tmg_official_reserve_deploy_transition_v2",
    executorId: OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_ID,
    executorVersion: OFFICIAL_RESERVE_DEPLOY_V2_EXECUTOR_VERSION,
    state,
    action: clone(actionInput),
    rulesTruth: "official_current_start_v2_phase_v1_reserve_deploy_exact_subset_v2",
    frozenSemanticKernel:
      `${OFFICIAL_RESERVE_DEPLOY_EXECUTOR_ID}@${OFFICIAL_RESERVE_DEPLOY_EXECUTOR_VERSION}`,
    silentCompatibilityUsed: false,
    trainingTruth: false,
