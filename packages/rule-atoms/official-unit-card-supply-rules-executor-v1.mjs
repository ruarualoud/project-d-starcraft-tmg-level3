import { isDeepStrictEqual } from "node:util";

import { hashStarcraftTmgContract } from
  "../authoritative-engine/referee-crypto-v1.mjs";
import {
  OFFICIAL_DEVELOPMENT_TRANCHE_DATASET_HASH,
  OFFICIAL_DEVELOPMENT_TRANCHE_SNAPSHOT_HASH,
  OFFICIAL_DEVELOPMENT_TRANCHE_SOURCE_LOCK_HASH,
} from "../source-data/official-development-tranche-source-lock-v1.mjs";
import { verifyOfficialUnitCardSupplyDataBundleV1 } from
  "../source-data/official-unit-card-supply-data-bundle-v1.mjs";
import {
  certifyOfficialUnitCardSupplyPlanV1,
  officialUnitCardSupplyProcedureKindsV1,
} from "./official-unit-card-supply-rules-kernel-v1.mjs";

export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ID =
  "authority.unit-card-supply-rules-v1";
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_VERSION = "1.0.0";
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_TRANSITION_SCHEMA =
  "starcraft_tmg_authority_v2.receipt";
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_TYPE =
  "resolve_unit_card_supply_rules_procedure";
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_PARAMETER_KIND =
  "official_unit_card_supply_rules_choice_v1";
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_PENDING_SCHEMA =
  "starcraft_tmg_official_unit_card_supply_rules_pending_v1";

export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_NEW_ATOM_IDS = Object.freeze([
  "rule-atom:singleton:core-11-supply-value-current-model-count:040328b3c8fb",
  "rule-atom:singleton:core-11-supply-value-rule-uses:f7e678ae1ad6",
  "rule-atom:singleton:core-11-supply-value-starting-slots:95ebd6e9560d",
  "rule-atom:singleton:core-5-1-phase-boxes:e16d4a44eb7d",
  "rule-atom:singleton:core-5-1-speed-null-value:84455f9124be",
  "rule-atom:singleton:core-5-1-supply-profile:5e9638b5269c",
  "rule-atom:singleton:core-5-2-base-diameter:0fa9d0018ed6",
  "rule-atom:singleton:core-5-2-combat-range:47018e3946cc",
  "rule-atom:singleton:core-5-2-upgrade-side:1e771d0623d2",
  "rule-atom:supply-profile-field-definition",
  "rule-atom:unit-army-slot-requirement",
  "rule-atom:unit-faction-tag-field",
].sort());
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_ATOM_IDS =
  OFFICIAL_UNIT_CARD_SUPPLY_RULES_NEW_ATOM_IDS;
export const OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ATOM_IDS =
  OFFICIAL_UNIT_CARD_SUPPLY_RULES_NEW_ATOM_IDS;

const PROCEDURE_KINDS = new Set(officialUnitCardSupplyProcedureKindsV1());

function fail(code, detail = "") { throw new Error(detail ? `${code}:${detail}` : code); }
function object(value) { return Boolean(value) && typeof value === "object" && !Array.isArray(value); }
function clone(value) { return structuredClone(value); }
function without(value, keys) {
  return Object.fromEntries(Object.entries(value).filter(([key]) => !keys.includes(key)));
}
function hashBody(value, field) { return hashStarcraftTmgContract(without(value, [field])); }

function verifySourceAndData(state, matchBinding = null) {
  const audit = state?.officialDevelopmentTrancheSourceLockAudit;
  const gameplay = state?.officialGameplayDataBundle;
  const bundle = state?.officialUnitCardSupplyDataBundle;
  if (!object(audit) || audit.lockHash !== OFFICIAL_DEVELOPMENT_TRANCHE_SOURCE_LOCK_HASH
    || audit.snapshotHash !== OFFICIAL_DEVELOPMENT_TRANCHE_SNAPSHOT_HASH
    || audit.normalizedDatasetHash !== OFFICIAL_DEVELOPMENT_TRANCHE_DATASET_HASH
    || audit.repositoryFallbackAllowed !== false || audit.trainingTruth !== false
    || gameplay?.sourceSnapshotHash !== OFFICIAL_DEVELOPMENT_TRANCHE_SNAPSHOT_HASH
    || gameplay?.normalizedDatasetHash !== OFFICIAL_DEVELOPMENT_TRANCHE_DATASET_HASH
    || gameplay?.repositoryFallbackAllowed !== false || gameplay?.trainingTruth !== false) {
    fail("UNIT_CARD_SUPPLY_SOURCE_LOCK_BINDING_INVALID");
  }
  verifyOfficialUnitCardSupplyDataBundleV1(bundle);
  if (matchBinding) {
    const expectedDataHash = hashStarcraftTmgContract(gameplay);
    if (matchBinding.dependencies?.dataSnapshot?.contentHash !== expectedDataHash
      || matchBinding.dataSnapshotHash !== expectedDataHash) {
      fail("UNIT_CARD_SUPPLY_DATA_ARTIFACT_BINDING_INVALID");
    }
  }
  return bundle;
}

function stateProjection(state, pending) {
  return hashStarcraftTmgContract({
    round: Number(state.round), phase: state.phase, activeSideKey: state.activeSideKey,
    players: state.players, pieces: state.pieces,
    unitCardSupplyRulesHistory: state.unitCardSupplyRulesHistory || [],
    lastUnitCardSupplyRulesResolution: state.lastUnitCardSupplyRulesResolution || null,
    officialUnitCardSupplyDataBundle: state.officialUnitCardSupplyDataBundle,
    pending: without(pending, ["pendingHash", "stateProjectionHash"]),
    trainingTruth: false,
  });
}

export function openOfficialUnitCardSupplyRulesPendingV1(stateInput, procedure = {}) {
  const state = clone(stateInput); const bundle = verifySourceAndData(state);
  const procedureKind = String(procedure.procedureKind || "");
  const sideKey = String(procedure.sideKey || state.activeSideKey || "");
  if (state.rulesProcedureMode !== true || state.pendingAction
    || !PROCEDURE_KINDS.has(procedureKind) || !object(state.players?.[sideKey])
    || procedure.candidatePlansComplete !== true
    || procedure.rulesDenominatorComplete !== true
    || !Array.isArray(procedure.candidatePlans)
    || procedure.candidatePlans.length === 0 || procedure.candidatePlans.length > 64) {
    fail("UNIT_CARD_SUPPLY_PROCEDURE_CERTIFICATE_REQUIRED");
  }
  const choices = procedure.candidatePlans.map((plan) => (
    certifyOfficialUnitCardSupplyPlanV1({ plan: { ...plan, sideKey }, procedureKind,
      unitCardSupplyDataBundle: bundle })
  ));
  if (new Set(choices.map((entry) => entry.planId)).size !== choices.length) {
    fail("UNIT_CARD_SUPPLY_PLAN_ID_DUPLICATE");
  }
  const body = {
    schema: OFFICIAL_UNIT_CARD_SUPPLY_RULES_PENDING_SCHEMA,
    stage: "choose_certified_unit_card_supply_plan",
    round: Number(state.round), phase: state.phase, sideKey, procedureKind,
    choices: choices.map((entry) => ({ ...entry,
      choiceId: `unit-card-supply-${entry.planHash}` }))
      .sort((left, right) => left.choiceId.localeCompare(right.choiceId)),
    rulesCertificate: { candidatePlansComplete: true, rulesDenominatorComplete: true,
      registryAuthority: "official_unit_card_supply_rules_kernel_v1",
      exactOfficialUnitProfilesBound: true,
      currentSupplySelectedByCurrentModelCount: true,
      nullSpeedForbidsAllMoveAndReposition: true,
      fullArmyEligibilityDeferredToSlice: 102,
      clientSuppliedLayoutSupplySlotsOrMobilityAccepted: false },
    sourceLockHash: OFFICIAL_DEVELOPMENT_TRANCHE_SOURCE_LOCK_HASH,
    unitCardSupplyDataBundleHash: bundle.bundleHash, stateProjectionHash: "",
    sourceRefreshPerformed: false, repositoryFallbackUsed: false, trainingTruth: false,
  };
  body.stateProjectionHash = stateProjection(state, body);
  state.pendingAction = { ...body, pendingHash: hashStarcraftTmgContract(body) };
  return { state, pending: clone(state.pendingAction) };
}

function verifyPending(state, matchBinding = null) {
  const bundle = verifySourceAndData(state, matchBinding); const pending = state?.pendingAction;
  if (state?.rulesProcedureMode !== true || !object(pending)
    || pending.schema !== OFFICIAL_UNIT_CARD_SUPPLY_RULES_PENDING_SCHEMA
    || pending.pendingHash !== hashBody(pending, "pendingHash")
    || pending.stateProjectionHash !== stateProjection(state, pending)
    || pending.unitCardSupplyDataBundleHash !== bundle.bundleHash
    || pending.rulesCertificate?.candidatePlansComplete !== true
    || pending.rulesCertificate?.rulesDenominatorComplete !== true
    || pending.rulesCertificate?.exactOfficialUnitProfilesBound !== true
    || pending.rulesCertificate?.currentSupplySelectedByCurrentModelCount !== true
    || pending.rulesCertificate?.nullSpeedForbidsAllMoveAndReposition !== true
    || pending.rulesCertificate?.fullArmyEligibilityDeferredToSlice !== 102
    || pending.rulesCertificate?.clientSuppliedLayoutSupplySlotsOrMobilityAccepted !== false
    || pending.sourceRefreshPerformed !== false || pending.repositoryFallbackUsed !== false
    || pending.trainingTruth !== false) fail("UNIT_CARD_SUPPLY_PENDING_INVALID");
  return pending;
}

function domainFor(state, options = {}) {
  const pending = verifyPending(state, options.matchBinding);
  const body = { schemaVersion: "starcraft_tmg_official_parameter_domain_v1",
    parameterKind: OFFICIAL_UNIT_CARD_SUPPLY_RULES_PARAMETER_KIND,
    matchBindingHash: String(options.matchBinding?.bindingHash || ""),
    round: pending.round, phase: pending.phase, sideKey: pending.sideKey,
    actionType: OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_TYPE, pieceId: "",
    executorId: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ID,
    executorVersion: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_VERSION,
    ruleAtomIds: [...OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_ATOM_IDS],
    parameterSchema: { type: "object", required: ["choiceId"],
      choiceId: { enum: pending.choices.map((entry) => entry.choiceId) },
      selectionOwner: "controlling_player" },
    constraints: { pendingHash: pending.pendingHash,
      stateProjectionHash: pending.stateProjectionHash,
      procedureKind: pending.procedureKind, choices: clone(pending.choices),
      clientSuppliedLayoutSupplySlotsOrMobilityAccepted: false },
    confirmationClass: "explicit_human",
    rulesTruth: "official_unit_card_supply_primitive_conformance",
    trainingTruth: false };
  return { ...body, domainId: `sc-domain-${hashStarcraftTmgContract(body)}` };
}

export function enumerateOfficialUnitCardSupplyRulesV1(state, options = {}) {
  const sideKey = String(options.sideKey || state?.activeSideKey || "");
  const candidates = []; const parameterDomains = [];
  try {
    const domain = domainFor(state, options);
    if (sideKey !== domain.sideKey) fail("UNIT_CARD_SUPPLY_WRONG_SIDE");
    parameterDomains.push(domain);
  } catch (error) {
    if (!options.includeDisabled) return { candidates, parameterDomains };
    candidates.push({ actionType: OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_TYPE,
      sideKey, phase: state?.phase || "", pieceId: "",
      executorId: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ID,
      executorVersion: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_VERSION,
      ruleAtomIds: [...OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_ATOM_IDS], isEnabled: false,
      disabledReason: String(error?.message || error).split(":")[0], score: 0,
      details: { trainingTruth: false } });
  }
  return { candidates, parameterDomains };
}

export function instantiateOfficialUnitCardSupplyRulesV1(state, domain,
  parameters, options = {}) {
  const expected = domainFor(state, options);
  if (!isDeepStrictEqual(domain, expected)) fail("UNIT_CARD_SUPPLY_PARAMETER_DOMAIN_STALE");
  if (!object(parameters) || Object.keys(parameters).length !== 1
    || typeof parameters.choiceId !== "string"
    || !expected.constraints.choices.some((entry) => entry.choiceId === parameters.choiceId)) {
    fail("UNIT_CARD_SUPPLY_CHOICE_INVALID");
  }
  const choice = expected.constraints.choices.find((entry) => (
    entry.choiceId === parameters.choiceId));
  return { action: { actionType: OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_TYPE,
    sideKey: expected.sideKey, phase: expected.phase, pieceId: "",
    unitCardSupplyPlan: { schema: "starcraft_tmg_official_unit_card_supply_rules_plan_v1",
      choiceId: choice.choiceId, planHash: choice.planHash,
      procedureKind: choice.procedureKind, pendingHash: state.pendingAction.pendingHash },
    domainId: domain.domainId,
    ruleAtomIds: [...OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_ATOM_IDS],
    executorId: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ID,
    executorVersion: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_VERSION,
    isEnabled: true, disabledReason: "", score: 1,
    details: { procedureKind: choice.procedureKind,
      resultHash: choice.result.resultHash, trainingTruth: false } },
  canonicalParameters: { choiceId: choice.choiceId } };
}

function contractAction(value) {
  return without(value, ["isEnabled", "disabledReason", "score", "details"]);
}

export function applyOfficialUnitCardSupplyRulesV1(stateInput, actionInput,
  options = {}) {
  if (!object(actionInput)
    || actionInput.actionType !== OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_TYPE
    || actionInput.executorId !== OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ID
    || actionInput.executorVersion !== OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_VERSION
    || !isDeepStrictEqual([...(actionInput.ruleAtomIds || [])].sort(),
      [...OFFICIAL_UNIT_CARD_SUPPLY_RULES_ACTION_ATOM_IDS])) {
    fail("UNIT_CARD_SUPPLY_ACTION_INVALID");
  }
  const domain = domainFor(stateInput, options);
  const choiceId = actionInput.unitCardSupplyPlan?.choiceId;
  const expected = instantiateOfficialUnitCardSupplyRulesV1(stateInput, domain,
    { choiceId }, options);
  if (!isDeepStrictEqual(contractAction(actionInput), contractAction(expected.action))) {
    fail("UNIT_CARD_SUPPLY_ACTION_STALE");
  }
  const pending = verifyPending(stateInput, options.matchBinding);
  const choice = pending.choices.find((entry) => entry.choiceId === choiceId);
  const state = clone(stateInput); state.pendingAction = null;
  const resolution = { schema: "starcraft_tmg_official_unit_card_supply_rules_resolution_v1",
    sideKey: pending.sideKey, procedureKind: choice.procedureKind,
    planId: choice.planId, planHash: choice.planHash, result: clone(choice.result),
    clientSuppliedLayoutSupplySlotsOrMobilityAccepted: false, trainingTruth: false };
  state.lastUnitCardSupplyRulesResolution = resolution;
  state.unitCardSupplyRulesHistory = Array.isArray(state.unitCardSupplyRulesHistory)
    ? state.unitCardSupplyRulesHistory : [];
  state.unitCardSupplyRulesHistory.push(clone(resolution));
  const event = { type: "unit_card_supply_rules_resolved", sideKey: pending.sideKey,
    procedureKind: choice.procedureKind, result: clone(resolution), trainingTruth: false };
  state.log = Array.isArray(state.log) ? state.log : [];
  state.log.push({ type: "unit_card_supply_rules_resolution", round: Number(state.round),
    phase: state.phase, sideKey: pending.sideKey, action: clone(actionInput),
    events: [clone(event)], trainingTruth: false });
  return { ok: true,
    schemaVersion: "starcraft_tmg_official_unit_card_supply_rules_transition_v1",
    executorId: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_ID,
    executorVersion: OFFICIAL_UNIT_CARD_SUPPLY_RULES_EXECUTOR_VERSION,
    state, events: [event], action: clone(actionInput), settlementRequired: false,
    rulesTruth: "official_unit_card_supply_primitive_resolved", trainingTruth: false };
}
