...
export const OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_ID =
  "authority.stimpack-move-consumer-v3";
export const OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_VERSION = "3.0.0";
export const OFFICIAL_STIMPACK_MOVE_V3_TRANSITION_SCHEMA =
  "starcraft_tmg_authority_v2.receipt";
export const OFFICIAL_STIMPACK_MOVE_V3_PARAMETER_KIND =
  "official_stimpack_move_path_v3";
export const OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_ATOM_IDS = Object.freeze([
  ...OFFICIAL_STIMPACK_MOVE_EXECUTOR_ATOM_IDS,
]);
export const OFFICIAL_STIMPACK_MOVE_V3_ACTION_ATOM_IDS = Object.freeze([
  ...OFFICIAL_STIMPACK_MOVE_ACTION_ATOM_IDS,
]);
...
const PATH_SCHEMA = "starcraft_tmg_official_stimpack_move_path_v3";
const PLAN_SCHEMA = "starcraft_tmg_official_stimpack_move_plan_v3";
...
function runtimeHash(matchBinding) {
  ...
  if (!HASH_PATTERN.test(value)) fail("STIMPACK_MOVE_V3_RUNTIME_BINDING_REQUIRED");
  return value;
}

function milli(value, code = "STIMPACK_MOVE_V3_GEOMETRY_INVALID", detail = "") {
...
function point(model) {
  return {
    xMilliInches: milli(model?.xInches, "STIMPACK_MOVE_V3_MODEL_GEOMETRY_INVALID"),
    yMilliInches: milli(model?.yInches, "STIMPACK_MOVE_V3_MODEL_GEOMETRY_INVALID"),
  };
}
...
function safePoint(value, index) {
  if (!object(value)
    ...
    fail("STIMPACK_MOVE_V3_PATH_POINT_INVALID", String(index));
  }
  return {
...
function verifyBindings(state, matchBinding) {
  ...
  if (!object(matchBinding)
    ...
    fail("STIMPACK_MOVE_V3_LATEST_OFFICIAL_DATA_REQUIRED");
  }
...
function verifyModel(model, pieceId) {
  if (!object(model)
    ...
    fail("STIMPACK_MOVE_V3_MODEL_SCOPE_UNSUPPORTED", pieceId);
  }
  point(model);
}
...
function verifyPiece(piece, sideKey, selectedUpgrades, role) {
  ...
  if (!object(piece)
    ...
    || models.length !== 1) {
    fail("STIMPACK_MOVE_V3_UNIT_SCOPE_UNSUPPORTED", role);
  }
...
function contextFor(state, sideKey, matchBinding) {
  ...
  if (card.sideKey !== sideKey
    ...
    || card.face !== "up") {
    fail("STIMPACK_MOVE_V3_FULL_COST_UNAVAILABLE");
  }
  if (!Array.isArray(state.activeAbilityUseHistory)
    ...
    fail("STIMPACK_MOVE_V3_NAMED_ABILITY_ALREADY_USED_THIS_ROUND");
  }
  const abilityBody = {
  ...
  if (buffResolution.effectiveValue !== 10) fail("STIMPACK_MOVE_V3_EFFECTIVE_SPEED_DRIFT");
  return {
    ...bindings,
    lineage,
    piece,
...
function domainFor(state, context, matchBinding) {
  const core = {
    ...
    parameterKind: OFFICIAL_STIMPACK_MOVE_V3_PARAMETER_KIND,
    ...
    ruleAtomIds: [...OFFICIAL_STIMPACK_MOVE_V3_ACTION_ATOM_IDS],
    parameterSchema: {
      type: "object",
    ...
    constraints: {
      ...
      geometryScope:
        "current_single_model_marine_empty_terrain_round_base_stimpack_move_v3",
    },
    confirmationClass: "direct_gesture_plus_resource_payment",
    rulesTruth: "official_current_stimpack_move_parameter_domain_v3",
    trainingTruth: false,
  };
...
function diagnosticAction(sideKey, pieceId, error) {
  return {
    ...
    score: 0,
    details: {
      rulesTruth: "official_stimpack_move_v3_fail_closed",
...
export function enumerateOfficialStimpackMoveV3(state, options = {}) {
...
function canonicalPath(start, raw) {
  if (!Array.isArray(raw) || raw.length === 0) fail("STIMPACK_MOVE_V3_PATH_REQUIRED");
  if (raw.length > MAX_RAW_PATH_POINTS) fail("STIMPACK_MOVE_V3_PATH_TOO_COMPLEX");
  const normalized = [start];
  for (const [index, value] of raw.entries()) {
    const next = safePoint(value, index);
    if (!pointEquals(normalized.at(-1), next)) normalized.push(next);
  }
  if (normalized.length < 2) fail("STIMPACK_MOVE_V3_PATH_MUST_CHANGE_POSITION");
  const points = [];
  for (const value of normalized) {
    while (points.length >= 2 && collinear(points.at(-2), points.at(-1), value)) points.pop();
    points.push(value);
  }
  if (points.length > MAX_CANONICAL_PATH_POINTS) fail("STIMPACK_MOVE_V3_PATH_TOO_COMPLEX");
...
function validatePath(domain, path) {
  if (path.distanceMilliInches > domain.constraints.maxDistanceMilliInches) {
    fail("STIMPACK_MOVE_V3_PATH_EXCEEDS_BUFFED_SPEED");
  }
  ...
  for (const endpoint of path.points.slice(1)) {
    if (endpoint.xMilliInches < radius
      ...
      fail("STIMPACK_MOVE_V3_PATH_OUTSIDE_BATTLEFIELD");
    }
  }
...
export function instantiateOfficialStimpackMoveV3(
  state,
  domain,
  parameters,
  options = {},
  ...
  if (!object(domain)
    || domain.parameterKind !== OFFICIAL_STIMPACK_MOVE_V3_PARAMETER_KIND
    || domain.executorId !== OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_ID
    || domain.executorVersion !== OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_VERSION) {
    fail("STIMPACK_MOVE_V3_PARAMETER_DOMAIN_INVALID");
  }
  const current = enumerateOfficialStimpackMoveV3(state, {
    sideKey: domain.sideKey,
  ...
  if (!expected || !isDeepStrictEqual(domain, expected)) {
    fail("STIMPACK_MOVE_V3_PARAMETER_DOMAIN_STALE");
  }
  ...
  return {
    ...
    rulesTruth: "official_current_stimpack_move_instantiation_v3",
    silentCompatibilityUsed: false,
    trainingTruth: false,
  };
}
...
export function applyOfficialStimpackMoveV3(stateInput, actionInput, options = {}) {
  if (!object(actionInput)
    || actionInput.actionType !== "move"
    || actionInput.executorId !== OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_ID
    || actionInput.executorVersion !== OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_VERSION
    || !object(actionInput.movePlan)) {
    fail("STIMPACK_MOVE_V3_ACTION_INVALID");
  }
  const enumeration = enumerateOfficialStimpackMoveV3(stateInput, {
  ...
  if (!domain) fail("STIMPACK_MOVE_V3_PARAMETER_DOMAIN_STALE");
  const instantiated = instantiateOfficialStimpackMoveV3(stateInput, domain, {
  ...
  if (!isDeepStrictEqual(actionInput, instantiated.action)) {
    fail("STIMPACK_MOVE_V3_ACTION_MISMATCH");
  }
  ...
  if (!card || card.readiness !== "ready" || card.face !== "up") {
    fail("STIMPACK_MOVE_V3_PAYMENT_CARD_STALE");
  }
  ...
  if (state.officialRoundSupplyState.roundSupplyStateHash !== previousSupplyHash
    ...
    fail("STIMPACK_MOVE_V3_SUPPLY_STATE_CHANGED");
  }
  const abilityEvent = {
    type: "use_ability",
  ...
  const moveEvent = {
    ...
    subtype: "current_stimpack_speed_consumer_v3",
    sideKey: actionInput.sideKey,
    pieceId: piece.id,
  ...
  return {
    ok: true,
    schemaVersion: "starcraft_tmg_official_stimpack_move_transition_v3",
    executorId: OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_ID,
    executorVersion: OFFICIAL_STIMPACK_MOVE_V3_EXECUTOR_VERSION,
    ...
    valueBuffResolution: clone(context.buffResolution),
    rulesTruth: "official_current_stimpack_before_move_speed_buff_exact_subset_v3",
    silentCompatibilityUsed: false,
    trainingTruth: false,
  };
}
