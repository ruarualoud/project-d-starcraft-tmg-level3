...
import { createOfficialExistingLifeSupportV2ContractClosureRuleSliceV1 } from
  "./official-existing-life-support-v2-contract-closure-rule-slice-v1.mjs";
...
import {
  ...
  OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_V2_NEW_ATOM_IDS,
...
export const OFFICIAL_SLICE_70_MIGRATED_ATOM_IDS = Object.freeze([
  ...OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_V2_NEW_ATOM_IDS,
...
function nextMajor(version) {
  ...
  if (!Number.isSafeInteger(major) || major < 1) {
    fail("GOLIATH_SCATTER_V2_CONTRACT_ATOM_VERSION_INVALID");
  }
  return `${major + 1}.0.0`;
}
...
function verifyPreviousSlice(previousSlice) {
  if (!object(previousSlice)
    ...
    fail("GOLIATH_SCATTER_V2_CONTRACT_PREVIOUS_SLICE_INVALID");
  }
  ...
  if (catalogueAudit.counts.byDisposition.executable !== 421
    ...
    fail("GOLIATH_SCATTER_V2_CONTRACT_PREVIOUS_RELEASE_DRIFT");
  }
}

function reboundAtom(atom) {
  const slug = atom.atomId.replace(/^rule-atom:/u, "goliath-scatter-v2:");
  return {
    ...clone(atom),
    ...
    rejectionCodes: [...new Set([
      ...atom.rejectionCodes,
      "GOLIATH_SCATTER_DATA_ADAPTER_V2_LATEST_UNIFIED_BUNDLE_REQUIRED",
      "GOLIATH_SCATTER_DATA_ADAPTER_V2_RECEIPT_INVALID",
      "GOLIATH_SCATTER_V2_ACTION_STALE",
    ])].sort((left, right) => left.localeCompare(right)),
    evidence: {
      positiveFixtureIds: [...atom.evidence.positiveFixtureIds,
        `${slug}:latest-official-sequential-batch-success`],
      ...
      interactionFixtureIds: [...atom.evidence.interactionFixtureIds,
        `${slug}:scatter-indirect-locked-full-cover-interaction`],
      lifecycleFixtureIds: [...atom.evidence.lifecycleFixtureIds,
        `${slug}:pending-sequence-damage-and-settlement-lifecycle`],
...
function createCurrentCatalogue(previousCatalogue) {
  if (OFFICIAL_SLICE_70_MIGRATED_ATOM_IDS.length !== 12) {
    fail("GOLIATH_SCATTER_V2_CONTRACT_TARGET_DENOMINATOR_MISMATCH");
  }
  const targetIds = new Set(OFFICIAL_SLICE_70_MIGRATED_ATOM_IDS);
  const observed = [];
  const atoms = previousCatalogue.atoms.map((atom) => {
    if (!targetIds.has(atom.atomId)) return clone(atom);
    if (atom.disposition !== "executable"
      || atom.effect?.executorId !== OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_EXECUTOR_ID) {
      fail("GOLIATH_SCATTER_V2_CONTRACT_TARGET_ASSIGNMENT_INVALID", atom.atomId);
    }
  ...
  const executorManifest = previousCatalogue.executorManifest
    .filter((entry) => entry.executorId !== OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_EXECUTOR_ID)
    .map(clone);
  ...
  return createRuleAtomCatalogue({
    ...
    catalogueVersion: "0.64.0-official-current-goliath-scatter-v2",
...
function buildEvidence(catalogue, { freezeRelease = true } = {}) {
  ...
  if ((freezeRelease && (
    ...
    || relationshipAudit.counts.declaredStateContractExecutors !== 36
    || relationshipAudit.counts.stateContractMissingExecutors !== 6
    ...
    fail("GOLIATH_SCATTER_V2_CONTRACT_EVIDENCE_INVALID", JSON.stringify({
      catalogueHash: catalogue.catalogueHash,
      runtimeHash: runtime.descriptor.runtimeHash,
...
export function createOfficialExistingGoliathScatterV2ContractClosureRuleSliceV1(
  input = {},
) {
  ...
  const body = {
    ...
    versionReassignedRuleAtomIds: [...OFFICIAL_SLICE_70_MIGRATED_ATOM_IDS],
    contractEvidenceReboundRuleAtomIds: [...OFFICIAL_SLICE_70_MIGRATED_ATOM_IDS],
    executableRuleAtomIds: catalogue.atoms
      .filter((atom) => atom.disposition === "executable")
      .map((atom) => atom.atomId),
    executorIds: [OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_V2_EXECUTOR_ID],
    executableScope: "current_goliath_scatter_ranged_batch_v2",
    existingExecutorContractClosureProgress: {
      ...
      legalCandidateCount: 32,
      exactSevenProfileSubsetsRequired: true,
      sequentialOneTwoThreeBatchProtocolRequired: true,
      indirectFireLockedInAndFullCoverDeclared: true,
      damageCasualtyPendingAndSettlementWritesDeclared: true,
      ...
      strictCompleteAtomCountBefore: 379,
      strictCompleteAtomCountAfter: 391,
      partialContractAtomCountBefore: 4,
      partialContractAtomCountAfter: 15,
      noContractAtomCountBefore: 38,
      noContractAtomCountAfter: 15,
      declaredStateContractExecutorCountBefore: 35,
      declaredStateContractExecutorCountAfter: 36,
      stateContractMissingExecutorCountBefore: 7,
      stateContractMissingExecutorCountAfter: 6,
    },
    ...
    ruleRelationshipProgress: {
      goliathScatterV2ContractComplete: true,
      historicalV1RuntimeAndRulesDisplayRetained: true,
      strictCompleteAtomCount: 391,
      partialContractAtomCount: 15,
      noContractAtomCount: 15,
      nonStrictAtomCount: 30,
      declaredStateContractExecutorCount: 36,
      stateContractMissingExecutorCount: 6,
      globalRelationshipCoverageComplete: false,
      derivedAuditIsRulesAuthority: false,
      productionEligible: false,
    },
    sliceForecast: {
      repairBatchOrdinal: 70,
      repairBatchOrdinalIsAtomIndex: false,
      completedBeforeThisSlice: 69,
      completedAfterThisSlice: 70,
      migratedExistingAtomCount: 12,
      newlyStrictAtomCount: 12,
      existingNonStrictAtomsBeforeThisSlice: 42,
      existingNonStrictAtomsAfterThisSlice: 30,
      remainingActionableAtomsBeforeThisSlice: 491,
      remainingActionableAtomsAfterThisSlice: 491,
      stateContractMissingExecutorsBeforeThisSlice: 7,
      stateContractMissingExecutorsAfterThisSlice: 6,
    ...
    historicalCompatibility: {
      ...
      frozenExecutorIds: [OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_EXECUTOR_ID],
      replacementExecutorIds: [OFFICIAL_GOLIATH_SCATTER_RANGED_BATCH_V2_EXECUTOR_ID],
    ...
    ctx2skill: {
      ...
      crossTimeReplayResult:
        "current_goliath_scatter_v2_plus_frozen_v1_history_required",
      promotions: [],
      blocks: [
        "no_skill_generation_or_promotion_in_rule_executor_contract_slice",
        "remaining_six_executor_contracts_open",
    ...
    harness: {
      ...
      uiTraceEvidence:
        "goliath_scatter_v2_authority_trace_only_browser_and_device_ui_pending",
      agentDecisionEvidence:
        "rules_owned_profile_subset_target_indirect_fire_locked_in_exact_action",
      memoryTraceEvidence: { refs: [], promotionAttempted: false },
      trainingTraceCandidates: [],
      rollbackOrDemotionRules: [
        "official_source_adapter_geometry_sequence_or_exact_action_drift_quarantines_v2",
        "replay_signature_relationship_or_historical_display_failure_demotes_runtime",
      ],
      userVisibleChecks: [
        "Goliath exposes 32 legal batches across seven weapon subsets",
        "Scatter attacks through full cover with off-LOS Evade",
        "Locked In adds six attacks only against the stationary target",
      ],
    },
    rulesEligible: false,
    ...
    trainingTruth: false,
    blocks: [
      "six_existing_executor_contracts_remain",
      "thirty_existing_atoms_remain_non_strict",
  ...
  if (freezeRelease && slice.sliceHash !== EXPECTED_SLICE_HASH) {
    fail("GOLIATH_SCATTER_V2_CONTRACT_SLICE_HASH_DRIFT", slice.sliceHash);
  }
  return freezeDeep(slice);
}

export function verifyOfficialExistingGoliathScatterV2ContractClosureRuleSliceV1(
  input = {},
) {
  ...
  if (!object(slice)
    ...
    fail("GOLIATH_SCATTER_V2_CONTRACT_SLICE_HASH_MISMATCH");
  }
  const expected = createOfficialExistingGoliathScatterV2ContractClosureRuleSliceV1({
  ...
  if (!isDeepStrictEqual(slice, expected)
    ...
    fail("GOLIATH_SCATTER_V2_CONTRACT_CONTENT_INVALID");
  }
  const evidence = buildEvidence(slice.catalogue, { freezeRelease: true });
  return freezeDeep({
    schema:
      "starcraft_tmg_official_existing_goliath_scatter_v2_contract_closure_audit_v1",
    valid: true,
    counts: {
      ...
      changedAtoms: OFFICIAL_SLICE_70_MIGRATED_ATOM_IDS.length,
