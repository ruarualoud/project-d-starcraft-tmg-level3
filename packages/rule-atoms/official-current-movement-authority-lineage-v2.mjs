...
import {
  ...
  OFFICIAL_START_OF_ROUND_V4_HISTORY_SCHEMA,
  OFFICIAL_START_OF_ROUND_V4_RESOLUTION_SCHEMA,
...
export const OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V2_SCHEMA =
  "starcraft_tmg_current_movement_authority_lineage_v2";
export const OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V2_CONTRACT =
  "start-of-round-v4-loadout+phase-initiative-v1+explicit-supply-mutation-lineage-v2";
...
const SUPPLY_MUTATION_EXECUTORS = Object.freeze(new Map([
  ["reserve_deployed", new Set(["authority.reserve-deploy-v4@4.0.0"])],
  ["unit_disengaged", new Set(["authority.disengage-v4@4.0.0"])],
  ["unit_disengage_failed", new Set(["authority.disengage-v4@4.0.0"])],
]));
...
function verifyStartOfRound(state, prefix) {
  ...
  if (!exactKeys(history, [
    ...
    || history.schema !== OFFICIAL_START_OF_ROUND_V4_HISTORY_SCHEMA
  ...
  const indexes = logs.flatMap((entry, index) => (
    Number(entry?.round) === Number(state.round)
      ...
      && entry?.action?.executorId === OFFICIAL_START_OF_ROUND_V4_EXECUTOR_ID
      && entry?.action?.executorVersion === OFFICIAL_START_OF_ROUND_V4_EXECUTOR_VERSION
      ? [index]
      : []
  ));
  if (indexes.length !== 1) {
    fail(prefix, "START_OF_ROUND_HANDOFF_INVALID", "current_v4_log");
  }
  ...
  if (!exactKeys(action, [
    ...
    || action.actionType !== OFFICIAL_START_OF_ROUND_V4_ACTION_TYPE
    ...
    || !isDeepStrictEqual(action.ruleAtomIds, [...OFFICIAL_START_OF_ROUND_V4_EXECUTOR_ATOM_IDS])
    || !hashBound(resolution, "startOfRoundResolutionHash")
    || resolution.schema !== OFFICIAL_START_OF_ROUND_V4_RESOLUTION_SCHEMA
    ...
    fail(prefix, "START_OF_ROUND_HANDOFF_INVALID", "current_v4_material");
  }
...
export function verifyOfficialCurrentMovementAuthorityLineageV2(state, options = {}) {
  const errorPrefix = String(options.errorPrefix || "CURRENT_MOVEMENT_V2").trim();
  if (!/^[A-Z][A-Z0-9_]*$/u.test(errorPrefix)) {
    throw new Error("CURRENT_MOVEMENT_V2_ERROR_PREFIX_INVALID");
  }
  ...
  const body = {
    schema: OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V2_SCHEMA,
    contract: OFFICIAL_CURRENT_MOVEMENT_AUTHORITY_LINEAGE_V2_CONTRACT,
    round: Number(state.round),
    phase: "movement",
