...
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_EXECUTOR_ID =
  "authority.marine-multi-enemy-casualty-close-combat-v4";
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_EXECUTOR_VERSION =
  "4.0.0";
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_TRANSITION_SCHEMA =
  "starcraft_tmg_authority_v3.receipt";
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_ATTACK_ACTION_TYPE =
  "fight";
export const OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_CASUALTIES_ACTION_TYPE =
  "resolve_multi_enemy_close_combat_casualties";
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_PENDING_SCHEMA =
  "starcraft_tmg_official_marine_multi_enemy_casualty_pending_v2";
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_NEW_ATOM_IDS =
  Object.freeze([]);
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_ACTION_ATOM_IDS =
  OFFICIAL_MARINE_MULTI_MODEL_CASUALTY_CLOSE_COMBAT_ACTION_ATOM_IDS;
export const OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_EXECUTOR_ATOM_IDS =
...
function contextFor(state, sideKey, attackerPieceId, targetPieceId, matchBinding) {
  ...
  return {
    plan,
    piece: state.pieces.find((entry) => entry.id === attackerPieceId),
    coEngager: state.pieces.find((entry) => entry.id === plan.coEngagerPieceId),
    target: state.pieces.find((entry) => entry.id === targetPieceId),
  };
}
...
function attackAction(context) {
  ...
  return {
    actionType: OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_ATTACK_ACTION_TYPE,
    ...
    resolutionMode: "multi_enemy_engagement_casualty_choice",
    attackPlanHash: plan.planHash,
    chance: clone(plan.chance),
    ruleAtomIds: [...OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_ACTION_ATOM_IDS],
...
function casualtyAction(pending, selection) {
  return {
    actionType: OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_CASUALTIES_ACTION_TYPE,
...
function verifyPending(pending) {
  if (!object(pending)
    || pending.schema !== OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_PENDING_SCHEMA
    ...
    || !object(pending.attackResolution)
    ...
    fail("MARINE_MULTI_ENEMY_CASUALTY_PENDING_INVALID");
  }
  return pending;
}
...
function currentContext(state, pending, options) {
  ...
  if (!isDeepStrictEqual(context.plan, pending.attackPlan)) {
    fail("MARINE_MULTI_ENEMY_CASUALTY_PENDING_STATE_DRIFT");
  }
  return context;
}
...
export function isOfficialMarineMultiEnemyCasualtyCloseCombatPendingV4(state) {
  return state?.pendingAction?.schema === OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_PENDING_SCHEMA;
}

export function enumerateOfficialMarineMultiEnemyCasualtyCloseCombatV4(
  state,
  options = {},
) {
  const sideKey = String(options.sideKey || state?.activeSideKey || "").trim();
  if (!SIDE_KEYS.includes(sideKey)) {
    if (options.throwOnError === true) fail("MARINE_MULTI_ENEMY_CASUALTY_SIDE_REQUIRED");
    return [];
  }
  try {
    if (isOfficialMarineMultiEnemyCasualtyCloseCombatPendingV4(state)) {
      ...
      if (state.phase !== "combat"
        ...
        || sideKey !== pending.ownerSideKey) {
        fail("MARINE_MULTI_ENEMY_CASUALTY_PENDING_WRONG_SEAT");
      }
        ...
        try {
          ...
          rows.push({
            ...
            details: {
              ...
              rulesTruth: "official_current_multi_enemy_engagement_casualty_consumer",
              trainingTruth: false,
            },
          });
        } catch (error) {
...
function normalizeReveals(reveals, chance) {
  if (!Array.isArray(reveals) || reveals.length !== chance.count) {
    fail("MARINE_MULTI_ENEMY_CASUALTY_REVEALS_REQUIRED");
  }
  return reveals.map((reveal) => {
    ...
    if (faces !== chance.faces
      ...
      fail("MARINE_MULTI_ENEMY_CASUALTY_REVEAL_INVALID");
    }
    return outcome;
  });
}
...
function ordinaryResolution(context, reveals) {
  ...
  const body = {
    schema: "starcraft_tmg_official_marine_multi_enemy_attack_resolution_v4",
    ...
    precision: { available: false, reason: "slice48_stimpack_scope_pending" },
    armour: {
      dice: hits,
      rolls: armourRolls,
    ...
    incomingDamage: (hits - saves) * context.plan.weapon.damage,
    trainingTruth: false,
  };
...
function transition(state, action, events, options, extras = {}) {
  return {
    ok: true,
    schemaVersion: OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_TRANSITION_SCHEMA,
    executorId: OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_EXECUTOR_ID,
    executorVersion: OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_CLOSE_COMBAT_EXECUTOR_VERSION,
    ...
    events,
    action,
    rulesTruth: extras.rulesTruth || "official_multi_enemy_casualty_resolution_v4",
    trainingTruth: false,
    ...extras,
  };
}
...
function openCasualtyChoice(
  ...
  const body = {
    schema: OFFICIAL_MARINE_MULTI_ENEMY_CASUALTY_PENDING_SCHEMA,
  ...
  state.activeSideKey = pending.ownerSideKey;
  const events = [{
    type: "marine_multi_enemy_casualty_choice_opened",
    sideKey: pending.ownerSideKey,
  ...
  return transition(state, expected, events, options, {
    ...
    attackResolution,
    casualtyDomain,
    rulesTruth: "official_defender_owned_multi_enemy_casualty_choice_opened",
  });
}
...
function finishCasualties(
  ...
  const event = {
    ...
    subtype: "marine_multi_enemy_engagement_casualty_consumer",
  ...
  return transition(state, expected, events, options, {
    ...
    rulesTruth: "official_current_multi_enemy_engagement_casualty_settlement",
  });
}
...
function resolveCasualtyChoice(stateInput, action, expected, options) {
  ...
  if (!selection || !isDeepStrictEqual(selection.casualtyModelIds, action.casualtyModelIds)) {
    fail("MARINE_MULTI_ENEMY_CASUALTY_SELECTION_STALE");
  }
  return finishCasualties(
    stateInput,
    expected,
...
export function applyOfficialMarineMultiEnemyCasualtyCloseCombatV4(
  stateInput,
  actionInput,
  options = {},
) {
  if (!object(actionInput)
    ...
    fail("MARINE_MULTI_ENEMY_CASUALTY_ACTION_INVALID");
  }
  const expected = enumerateOfficialMarineMultiEnemyCasualtyCloseCombatV4(stateInput, {
    sideKey: actionInput.sideKey,
    matchBinding: options.matchBinding,
    throwOnError: true,
  }).map(actionFromCandidate).find((candidate) => isDeepStrictEqual(candidate, actionInput));
  if (!expected) fail("MARINE_MULTI_ENEMY_CASUALTY_ACTION_STALE");
  if (actionInput.actionType === OFFICIAL_RESOLVE_MARINE_MULTI_ENEMY_CASUALTIES_ACTION_TYPE) {
