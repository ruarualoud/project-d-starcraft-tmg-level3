...
export const OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_ID =
  "authority.attack-resolution-kernel-v4";
export const OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_VERSION = "4.0.0";
...
const ENGAGED_RANGED_EVADE_REASON =
...
function verifyAntiEvade(antiEvade) {
  if (!antiEvade) return;
  const modifier = Number(antiEvade.parameters?.evadeThresholdModifier);
  if (!Number.isSafeInteger(modifier) || modifier >= 0) {
    fail("ATTACK_ANTI_EVADE_PARAMETERS_INVALID");
  }
}

function verifyBurstFire(burstFire) {
  if (!burstFire) return;
  const maximumDistanceInches = Number(burstFire.parameters?.maximumDistanceInches);
  const additionalRateOfAttack = Number(burstFire.parameters?.additionalRateOfAttack);
  if (!Number.isSafeInteger(maximumDistanceInches)
    || maximumDistanceInches <= 0
    || !Number.isSafeInteger(additionalRateOfAttack)
    || additionalRateOfAttack <= 0) {
    fail("ATTACK_BURST_FIRE_PARAMETERS_INVALID");
  }
}
...
function verifyProfile(profile) {
  ...
  verifyAntiEvade(effectById(profile, "attack-effect:anti-evade-v1"));
  verifyBurstFire(effectById(profile, "attack-effect:burst-fire-v1"));
}

function normalizedTarget(target) {
  const armourThreshold = Number(target?.armourThreshold);
  const rawEvadeThreshold = target?.evadeThreshold;
  const evadeThreshold = rawEvadeThreshold === null
    ? null
    : Number(rawEvadeThreshold);
  ...
  if (!Number.isSafeInteger(armourThreshold)
    ...
    || armourThreshold > 6
    || (evadeThreshold !== null && (
      !Number.isSafeInteger(evadeThreshold)
        || evadeThreshold < 2
        || evadeThreshold > 6
    ))
    || combatTags.length === 0) {
...
function normalizedEvadeEligibility(value) {
  if (!object(value) || typeof value.eligible !== "boolean") {
    fail("ATTACK_EVADE_ELIGIBILITY_REQUIRED");
  }
  const reason = String(value.reason || "");
  if ((value.eligible === true && reason !== ENGAGED_RANGED_EVADE_REASON)
    || (value.eligible === false && reason !== "none")) {
    fail("ATTACK_EVADE_ELIGIBILITY_UNSUPPORTED", reason);
  }
...
function planAttack(input) {
  ...
  const evadeEligibility = normalizedEvadeEligibility(input?.evadeEligibility);
  if (evadeEligibility.eligible && target.evadeThreshold === null) {
    fail("ATTACK_EVADE_NULL_VALUE");
  }
  ...
  const antiEvade = effectById(profile, "attack-effect:anti-evade-v1");
  const burstFire = effectById(profile, "attack-effect:burst-fire-v1");
  const burstFireApplied = Boolean(burstFire)
    && distanceInches <= Number(burstFire.parameters.maximumDistanceInches);
  const printedRateOfAttack = Number(profile.rateOfAttack);
  if (!Number.isSafeInteger(printedRateOfAttack) || printedRateOfAttack <= 0) {
    fail("ATTACK_RATE_OF_ATTACK_INVALID");
  }
  const effectiveRateOfAttack = printedRateOfAttack + (
    burstFireApplied ? Number(burstFire.parameters.additionalRateOfAttack) : 0
  );
  if (!Number.isSafeInteger(effectiveRateOfAttack) || effectiveRateOfAttack <= 0) {
    fail("ATTACK_EFFECTIVE_RATE_OF_ATTACK_INVALID");
  }
  const modifier = antiEvade
    ? Number(antiEvade.parameters.evadeThresholdModifier)
    : 0;
  const targetNumberDelta = evadeEligibility.eligible ? -modifier : 0;
  const unboundedThreshold = evadeEligibility.eligible
    ? target.evadeThreshold + targetNumberDelta
    : null;
  const effectiveThreshold = evadeEligibility.eligible
    ? Math.max(2, Math.min(6, unboundedThreshold))
    : null;
  const layout = {
    hit: effectiveRateOfAttack,
    surge: surge ? 1 : 0,
    armour: effectiveRateOfAttack,
    evade: evadeEligibility.eligible ? effectiveRateOfAttack : 0,
  };
  const body = {
    schema: "starcraft_tmg_official_attack_resolution_plan_v4",
    kernelId: OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_ID,
    kernelVersion: OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_VERSION,
    ...
    burstFire: burstFire ? {
      effectAtomId: burstFire.effectAtomId,
      maximumDistanceInches: burstFire.parameters.maximumDistanceInches,
      additionalRateOfAttack: burstFire.parameters.additionalRateOfAttack,
      applied: burstFireApplied,
    } : null,
    effectAtomIds: profile.effects.map((entry) => entry.effectAtomId),
    evade: {
      eligible: evadeEligibility.eligible,
      eligibilityReason: evadeEligibility.reason,
      baseThreshold: target.evadeThreshold,
      modifier,
      targetNumberDelta,
      unboundedThreshold,
      effectiveThreshold,
      ...
      minimumTargetNumber: 2,
      maximumTargetNumber: 6,
...
function resolveAttack(plan, reveals) {
  if (!object(plan)
    || plan.schema !== "starcraft_tmg_official_attack_resolution_plan_v4"
    || plan.kernelId !== OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_ID
    || plan.kernelVersion !== OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_VERSION
  ...
  const damagePerDie = pierceMatched
    ...
    : plan.profile.damage;
  const antiEvade = effectById(plan.profile, "attack-effect:anti-evade-v1");
  const stages = {
    declaration: {
    ...
    effects: {
      ...
      burstFireApplied: plan.burstFire?.applied === true,
      burstFireMaximumDistanceInches: plan.burstFire?.maximumDistanceInches || null,
      burstFireAdditionalRateOfAttack: plan.burstFire?.additionalRateOfAttack || 0,
      printedRateOfAttack: plan.printedRateOfAttack,
      effectiveRateOfAttack: plan.effectiveRateOfAttack,
      surgeRolls,
      surgeResults,
      surgeMatched,
      bypassedArmourHits,
      ...
      antiEvadeApplied: Boolean(antiEvade) && plan.evade.eligible,
      antiEvadeModifier: antiEvade?.parameters.evadeThresholdModifier || 0,
      antiEvadeTargetNumberDelta: plan.evade.targetNumberDelta,
    },
    armour: {
      dice: armourDice,
    ...
    evade: {
      eligible: plan.evade.eligible,
      eligibilityReason: plan.evade.eligibilityReason,
      dice: evadeDice,
      rolls: resolvedEvadeRolls,
      unusedPreallocatedRolls: evadeRolls.slice(evadeDice),
      baseThreshold: plan.evade.baseThreshold,
      modifier: plan.evade.modifier,
      targetNumberDelta: plan.evade.targetNumberDelta,
      unboundedThreshold: plan.evade.unboundedThreshold,
      effectiveThreshold: plan.evade.effectiveThreshold,
      modifierAppliedBeforeRoll: true,
      modifiesDieResult: false,
      naturalFailures: resolvedEvadeRolls.filter((roll) => roll === 1),
      naturalSuccesses: resolvedEvadeRolls.filter((roll) => roll === 6),
    ...
    damage: {
      ...
      damagePerDie,
      totalDamage: confirmedDamageDice * damagePerDie,
    },
  };
  const body = {
    schema: "starcraft_tmg_official_attack_resolution_receipt_v4",
    kernelId: OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_ID,
    kernelVersion: OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_VERSION,
...
export function createOfficialAttackResolutionKernelV4() {
  const descriptorBody = {
    schema: "starcraft_tmg_official_attack_resolution_kernel_descriptor_v4",
    kernelId: OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_ID,
    kernelVersion: OFFICIAL_ATTACK_RESOLUTION_KERNEL_V4_VERSION,
    stages: ["declaration", "hit", "effects", "armour", "evade", "damage"],
    supportedEffectAtomIds: [...SUPPORTED_EFFECT_ATOM_IDS],
    evadeEligibilityReasons: [ENGAGED_RANGED_EVADE_REASON],
    rateOfAttackPolicy: {
      printedRateOfAttackIsImmutable: true,
      effectsModifyEffectiveRateOfAttack: true,
      chanceLayoutUsesEffectiveRateOfAttack: true,
    },
    evadeModifierPolicy: {
      applyToTargetNumberBeforeRoll: true,
      modifyDieResult: false,
      minimumTargetNumber: 2,
      maximumTargetNumber: 6,
      naturalOneAlwaysFails: true,
      naturalSixAlwaysSucceeds: true,
